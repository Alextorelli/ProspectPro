...
const EDGE_FUNCTION_JWT = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNyaXljZWt4ZHFuZXNkc2d3aXVjIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTc5NjU3ODksImV4cCI6MjA3MzU0MTc4OX0.Rx_1Hjz2eayKie0RpPB28i7_683ZwhVJ_5Eu_rzTWpI';
...
class UserAwareProspectPro {
    ...
    updateUserStatus() {
        ...
        if (this.currentUser) {
            userStatus.innerHTML = `
                ...
                <div class="text-gray-600 text-sm">
                    <strong>User ID:</strong> ${this.currentUser.id.slice(0, 8)}...
                </div>
            ...
            userStatus.innerHTML = `
                ...
                <div class="text-gray-600 text-sm">
                    <strong>Session ID:</strong> ${this.sessionUserId.slice(-12)}
                </div>
                <div class="text-gray-500 text-xs mt-2">
    ...
    async loadRecentCampaigns() {
        try {
            ...
            if (error) {
                ...
                recentCampaigns.innerHTML = '<p class="text-red-500 text-sm">Error loading campaigns</p>';
                return;
            }
            ...
            recentCampaigns.innerHTML = campaigns.map(campaign => `
                <div class="border border-gray-200 rounded-lg p-3 hover:bg-gray-50 cursor-pointer transition-colors" 
                    ...
                    <div class="flex justify-between items-start">
                        <div class="flex-1">
                            <div class="font-medium text-sm text-gray-900">${campaign.business_type}</div>
                            <div class="text-xs text-gray-600 mt-1">${campaign.location}</div>
    ...
    async handleDiscovery(event) {
        ...
        try {
            ...
            const response = await fetch(`${SUPABASE_URL}/functions/v1/business-discovery-user-aware`, {
                method: 'POST',
                headers: {
    ...
    displayResults(result) {
        ...
        const avgConfidence = totalLeads > 0 ? 
            Math.round(result.leads.reduce((sum, lead) => sum + (lead.optimizedScore || 0), 0) / totalLeads) : 0;
        ...
        resultsContent.innerHTML = `
            ...
            <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-6 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div class="bg-white rounded-lg p-4 shadow-sm">
                        <div class="text-2xl font-bold text-purple-700">$${(result.optimization?.totalCost || 0).toFixed(2)}</div>
                        <div class="text-sm text-purple-600">Total Cost</div>
                    </div>
                    <div class="bg-white rounded-lg p-4 shadow-sm">
                        <div class="text-2xl font-bold text-gray-700">${result.optimization?.processingTime || 'N/A'}</div>
                ...
                <div class="flex">
                    <div class="ml-3">
                        <p class="text-sm text-yellow-700">
                            <strong>Campaign Owner:</strong> ${result.userManagement?.isAuthenticated ? 'Authenticated User' : 'Anonymous Session'} • 
                            <strong>Ownership:</strong> ${result.userManagement?.campaignOwnership || 'Session-based'}
                            ${!this.currentUser ? ' • Sign up to save permanently' : ''}
                        </p>
                    </div>
                </div>
            </div>
                        ...
                        ${(result.leads || []).map((lead, index) => `
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">${lead.businessName || 'N/A'}</div>
                                    <div class="text-sm text-gray-500">${lead.address || 'N/A'}</div>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-900">
                                    <div class="space-y-1">
                                        ${lead.phone ? `<div>📞 ${lead.phone}</div>` : ''}
                                        ${lead.website ? `<div>🌐 <a href="${lead.website}" target="_blank" class="text-blue-600 hover:underline">${lead.website}</a></div>` : ''}
                                        ${lead.email ? `<div>✉️ ${lead.email}</div>` : ''}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                        (lead.optimizedScore || 0) >= 80 ? 'bg-green-100 text-green-800' :
                                        (lead.optimizedScore || 0) >= 60 ? 'bg-yellow-100 text-yellow-800' :
    ...
    async handleExport() {
        ...
        try {
            ...
            const response = await fetch(`${SUPABASE_URL}/functions/v1/campaign-export-user-aware?download=true`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${EDGE_FUNCTION_JWT}`,
                    'Content-Type': 'application/json'
            ...
            a.href = url;
            a.download = `prospectpro_export_${this.lastCampaignId.slice(-8)}_${Date.now()}.csv`;
    ...
    async viewCampaign(campaignId) {
        ...
        this.showSuccess(`Campaign ${campaignId.slice(-8)} selected. Export functionality is now available.`);
    }
    ...
    async signIn() {
        try {
            ...
            if (error) {
                ...
                this.showSuccess('Welcome back! Your campaigns are now linked to your account.');
            }
    ...
    async signUp() {
        try {
            ...
            if (error) {
                ...
                this.showSuccess('Account created! Check your email for confirmation. You can continue using the app while we verify your email.');
            }
    ...
    async signOut() {
        try {
            ...
            this.showSuccess('Signed out successfully. You can continue as an anonymous user.');
