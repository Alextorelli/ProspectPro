...
async function testBusinessDiscovery(request: BusinessDiscoveryRequest) {
  ...
  const googleApiKey = env.GOOGLE_PLACES_API_KEY || Deno.env.get('GOOGLE_PLACES_API_KEY');
  if (!googleApiKey) {
  ...
  const placesUrl = `https://maps.googleapis.com/maps/api/place/textsearch/json?query=${encodeURIComponent(searchQuery)}&radius=${radius}&key=${googleApiKey}`;
...
async function runTests() {
  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      console.log(`✅ Success! Found ${result.qualifiedResults} qualified businesses out of ${result.totalFound} total`);
      console.log(`💰 Estimated cost: $${result.estimatedCost}`);
      console.log(`📊 Results preview:`);
      
      result.businesses.forEach((business, index) => {
        console.log(`   ${index + 1}. ${business.name} (${business.confidence}% confidence)`);
