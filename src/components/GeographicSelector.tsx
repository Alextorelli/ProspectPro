...
export const GeographicSelector: React.FC<GeographicSelectorProps> = ({
  onLocationChange,
  initialLocation = { lat: 40.7128, lng: -74.0060, address: 'New York, NY' },
  initialRadius = 10
}) => {
  ...
  const geocodeAddress = async (addressInput: string): Promise<Location | null> => {
    try {
      setIsGeocoding(true);
  ...
  const SimpleMapView: React.FC = () => {
    ...
    return (
      <div className="relative bg-gradient-to-br from-green-100 to-blue-100 rounded-lg overflow-hidden border border-gray-300" style={{ height: '300px' }}>
        {/* Map Background */}
        ...
        <div className="absolute inset-0 opacity-20">
          ...
          {[...Array(8)].map((_, i) => (
            <div key={`v-${i}`} className="absolute border-l border-gray-400" style={{ left: `${i * 12.5}%`, height: '100%' }} />
          ))}
        </div>
        ...
        <div className="absolute bottom-2 left-2 right-2 bg-white bg-opacity-90 rounded p-2">
          <div className="text-xs font-medium text-gray-800 truncate">{location.address}</div>
          <div className="text-xs text-gray-600">
            Radius: {radius} miles • Lat: {location.lat.toFixed(4)}, Lng: {location.lng.toFixed(4)}
          </div>
        </div>
  ...
  const FullMapView: React.FC = () => {
    return (
      <div className="relative bg-gray-100 rounded-lg border border-gray-300" style={{ height: '400px' }}>
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center">
            <div className="text-6xl mb-4">🗺️</div>
            <h3 className="text-lg font-medium text-gray-800 mb-2">Interactive Map</h3>
            <p className="text-sm text-gray-600 mb-4 max-w-md">
              Full interactive map integration requires Google Maps or Mapbox API.
              <br />Add API keys to enable:
            </p>
    ...
    <div className="space-y-4">
      ...
      <div className="p-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md">
        <div className="text-sm text-gray-700 dark:text-gray-300">
          <strong>Search Area:</strong> {radius}-mile radius around {location.address}
        </div>
        <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
          Approximate coverage: ~{Math.round(Math.PI * radius * radius)} square miles
        </div>
      </div>
    </div>
  );
