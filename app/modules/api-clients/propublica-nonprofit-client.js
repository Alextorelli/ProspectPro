...
class ProPublicaNonprofitClient {
  ...
  async searchNonprofits(searchTerm, options = {}) {
    ...
    // Check cache first
    const cacheKey = `propublica_search_${searchTerm
      .toLowerCase()
    ...
    try {
      ...
      if (
        ...
        normalizedResponse.organizations.length > 0
      ) {
        const enhanced = await this.enhanceWithFinancialData(
          normalizedResponse.organizations.slice(0, 5)
        );
  ...
  async getOrganizationByEIN(ein, options = {}) {
    ...
    try {
      const response = await this.makeRequest(
        `/organizations/${cleanEIN}.json`
      );
      const normalizedResponse = this.normalizeOrganizationResponse(
        response,
        cleanEIN
      );
      ...
      console.error(
        `Error fetching organization for EIN ${ein}:`,
        error.message
      );
  ...
  async getFilings(ein, limit = 5) {
    ...
    try {
      const response = await this.makeRequest(
        `/organizations/${cleanEIN}/filings.json`
      );
      ...
      try {
        ...
        if (response.status === 404) {
          ...
          if (attempt < retries) {
            ...
            console.log(
              `ProPublica rate limit hit. Waiting ${delay}ms before retry ${attempt}/${retries}`
            );
            await new Promise((resolve) => setTimeout(resolve, delay));
            continue;
          }
          throw new Error(
            "ProPublica rate limit exceeded - Please try again later"
          );
        } else if (response.status >= 500) {
          if (attempt < retries) {
            const delay = Math.pow(2, attempt) * 1000;
            console.log(
              `ProPublica server error ${response.status}. Retrying in ${delay}ms`
            );
            await new Promise((resolve) => setTimeout(resolve, delay));
            continue;
          }
  ...
  normalizeFilingsResponse(data, limit) {
    ...
    const filings = data.filings.slice(0, limit).map((filing) => ({
      ...
      totalProgramServiceExpenses:
        filing.total_program_service_expenses || null,
  ...
  getUsageStats() {
    return {
      ...
      rateLimitStatus: {
        ...
        nextAvailableTime:
          this.requestTimes.length >= this.rateLimitPerMinute
            ? new Date(
