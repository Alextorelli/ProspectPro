/**
 * California SOS Registry Provider
 */

const CaliforniaSOS = require("../../api-clients/california-sos-client");

class CaliforniaSOSProvider {
  constructor(apiKey = null) {
    this.client = new CaliforniaSOS(apiKey);
    this.name = "California SOS";
  }
  ...
  isRelevant(business, searchParams = {}) {
    // Check if business is in California
    const state = business.state || searchParams.state || "";
    const address = business.address || "";

    return (
      state.toLowerCase().includes("ca") ||
      state.toLowerCase().includes("california") ||
  ...
  /**
   * Validate business through California SOS
   */
  async validate(business, searchParams = {}) {
    const businessName = business.name || business.businessName || "";

    if (!businessName) {
      return { error: "No business name provided", source: "california_sos" };
    }
    ...
    try {
      ...
      return {
        ...
        registrationDate: result?.[0]?.registration_date || null,
  ...
  calculateConfidence(results, targetName) {
    ...
    results.forEach((result) => {
      const resultName = (result.entity_name || "")
        .toLowerCase()
        .replace(/[^\w\s]/g, "")
        .trim();
...
module.exports = CaliforniaSOSProvider;
