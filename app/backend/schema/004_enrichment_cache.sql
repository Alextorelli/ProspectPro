...
ALTER TABLE public.enrichment_cache
ALTER COLUMN cache_key
SET NOT NULL,
  ALTER COLUMN request_type
SET NOT NULL,
  ALTER COLUMN request_params
SET NOT NULL,
  ALTER COLUMN response_data
SET NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_enrichment_cache_active ON public.enrichment_cache(is_active)
WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_enrichment_cache_created ON public.enrichment_cache(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_enrichment_cache_tier ON public.enrichment_cache((request_params->>'tier'));
CREATE INDEX IF NOT EXISTS idx_enrichment_cache_strategy ON public.enrichment_cache((request_params->>'strategy'));
...
CREATE POLICY "enrichment_cache_service_access" ON public.enrichment_cache FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "enrichment_cache_stats_service_access" ON public.enrichment_cache_stats FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE OR REPLACE FUNCTION public.generate_cache_key(p_request_type TEXT, p_params JSONB) RETURNS TEXT LANGUAGE plpgsql IMMUTABLE SECURITY DEFINER
SET search_path = public AS $$ BEGIN RETURN encode(
    digest(
...
END;
$$;
CREATE OR REPLACE FUNCTION public.get_cached_response(p_request_type TEXT, p_params JSONB) RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public AS $$
DECLARE v_cache_key TEXT;
v_response JSONB;
BEGIN v_cache_key := public.generate_cache_key(p_request_type, p_params);
...
VALUES (CURRENT_DATE, p_request_type, 1, 1) ON CONFLICT (date, request_type) DO
UPDATE
SET cache_hits = public.enrichment_cache_stats.cache_hits + 1,
  total_requests = public.enrichment_cache_stats.total_requests + 1,
  hit_ratio = ROUND(
    (public.enrichment_cache_stats.cache_hits + 1.0) / (
      public.enrichment_cache_stats.total_requests + 1.0
    ) * 100,
    2
  ),
  updated_at = NOW();
...
VALUES (CURRENT_DATE, p_request_type, 1, 1) ON CONFLICT (date, request_type) DO
UPDATE
SET cache_misses = public.enrichment_cache_stats.cache_misses + 1,
  total_requests = public.enrichment_cache_stats.total_requests + 1,
  hit_ratio = ROUND(
...
VALUES (CURRENT_DATE, p_request_type, p_cost) ON CONFLICT (date, request_type) DO
UPDATE
SET total_cost = public.enrichment_cache_stats.total_cost + p_cost,
  updated_at = NOW();
RETURN v_cache_key;
END;
$$;
CREATE OR REPLACE FUNCTION public.cleanup_expired_cache() RETURNS INTEGER LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public AS $$
...
END;
$$;
CREATE OR REPLACE VIEW public.enrichment_cache_analytics WITH (security_invoker = true) AS
SELECT request_type,
...
CREATE OR REPLACE VIEW public.cache_performance_summary WITH (security_invoker = true) AS
SELECT date,
  ...
  ROUND(
    CASE
      WHEN SUM(COALESCE(total_requests, 0)) > 0 THEN SUM(COALESCE(cache_hits, 0))::DECIMAL / SUM(COALESCE(total_requests, 0)) * 100
      ELSE 0
    END,
    2
  ) AS daily_hit_ratio,
...
GROUP BY date
ORDER BY date DESC;
GRANT ALL ON public.enrichment_cache TO postgres,
  service_role;
GRANT ALL ON public.enrichment_cache_stats TO postgres,
  service_role;
GRANT SELECT ON public.enrichment_cache_analytics TO authenticated,
  service_role;
GRANT SELECT ON public.cache_performance_summary TO authenticated,
  service_role;
