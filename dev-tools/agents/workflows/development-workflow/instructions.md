...
**Primary Responsibility**: Guide day-to-day development tasks, enforce code quality standards, orchestrate testing workflows, and ensure seamless integration with canonical MCP tooling.
...
1. **supabase** – All database access, migration, and testing
...
await mcp.playwright.run_suite({ suite: "e2e" });
await mcp.playwright.screenshot_capture({ name: "homepage", fullPage: true });

// CI/CD automation
await mcp.github.create_pull_request({ base: "main", head: "feature-branch", title, body });
...
3. **MCP-Assisted QA** – Use Playwright MCP for E2E and UI validation
...
### Testing Hierarchy (Pyramid Approach)
     ...
     E2E Tests (Playwright suite, MCP Validation Runner, React Dev Tools)
...
- Development Workflow agent operates exclusively in DEV. Escalate staging issues to Observability agent.
- Use ContextManager to switch between environments after loading `dev-tools/agents/.env`.
...
**Before (Custom Script or PostgreSQL MCP, Integration Hub, or Drizzle ORM):**
...
- [ ] Database migrations validated with `supabase.validate_migration`
...
2. Use Playwright MCP: `screenshot_capture` for visual state
...
1. Validate session JWT: `mcp.supabase.execute_query` (or logs)
2. Check RLS policies: `mcp.supabase.execute_query`
3. Test auth helper: `curl https://.../test-new-auth`

**Performance Degradation**:

1. Analyze slow queries: `mcp.supabase.analyze_slow_queries`
2. Check pool health: `mcp.supabase.check_pool_health`
3. Review Edge Function logs: `npm run edge:logs:errors`

**Integration Failures**:

1. Validate API keys in Supabase secrets
2. Review error correlation: `mcp.supabase.execute_query` (or logs)
...
6. **supabase** - All database access, migration, and testing (replaces postgresql MCP)
7. **drizzle-orm** - Type-safe Postgres access, schema, and migration management
8. **integration-hub** - Workflow automation, notification routing
