...
async function runWellnessValidationTest() {
  ...
  try {
    ...
    csvExporter.initializeCampaign(campaignId, {
      ...
      description:
        "Single lead validation test for wellness business with complete pipeline processing",
    });
    ...
    results.slice(0, 3).forEach((business, index) => {
      ...
      console.log(
        `      Rating: ${business.rating}/5 (${business.user_ratings_total} reviews)`
      );
    ...
    if (enhancedResults.leads.length === 0) {
      throw new Error(
        "No qualified wellness leads found after enhanced processing"
      );
    }

    // Select the highest quality lead
    const topLead = enhancedResults.leads.sort(
      (a, b) =>
    ...
    console.log(
      `📊 Confidence Score: ${(
        topLead.finalConfidenceScore || topLead.confidenceScore
      ).toFixed(1)}%`
    );
    console.log(`📍 Address: ${topLead.address || topLead.formatted_address}`);
    console.log(
      `📞 Phone: ${topLead.phone || topLead.companyPhone || "Not available"}`
    );
    console.log(`🌐 Website: ${topLead.website || "Not available"}`);
    console.log(
      `📧 Email: ${topLead.email || topLead.companyEmail || "Not available"}`
    );
    console.log(
      `🗺️  Foursquare Enhanced: ${topLead.foursquareData ? "✅ Yes" : "❌ No"}`
    );
    console.log(
      `👥 Business Category: ${
        topLead.businessType || topLead.types?.[0] || "Wellness"
      }`
    );
    console.log(
      `⭐ Google Rating: ${topLead.googleRating || topLead.rating}/5`
    );
    ...
    console.log(
      `   Google Places Integration: ${
        pipelineValidation.hasGoogleData ? "✅" : "❌"
      }`
    );
    console.log(
      `   Foursquare Enhancement: ${
        pipelineValidation.hasFoursquareData ? "✅" : "❌"
      }`
    );
    console.log(
      `   Hunter.io Email Discovery: ${
        pipelineValidation.hasEmail ? "✅" : "❌"
      }`
    );
    console.log(
      `   Website Verification: ${pipelineValidation.hasWebsite ? "✅" : "❌"}`
    );
    console.log(
      `   Phone Contact: ${pipelineValidation.hasPhone ? "✅" : "❌"}`
    );

    const completenessScore =
      Object.values(pipelineValidation).filter(Boolean).length;
    console.log(
      `   Overall Completeness: ${completenessScore}/5 (${Math.round(
        (completenessScore / 5) * 100
      )}%)`
    );
    console.log("");
    ...
    console.log(
      `🔧 Pipeline Health: ${completenessScore}/5 components working`
    );
    console.log("");

    if (
      completenessScore >= 4 &&
      (topLead.finalConfidenceScore || topLead.confidenceScore) >= 70
    ) {
