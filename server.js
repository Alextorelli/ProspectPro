...
async function loadSecretsFromSupabase() {
  ...
  try {
    ...
    try {
      ...
      if (!vaultError && vaultSecrets?.length > 0) {
        ...
        vaultSecrets.forEach(({ name, decrypted_secret }) => {
          if (!process.env[name] && decrypted_secret && decrypted_secret !== 'CONFIGURE_IN_SUPABASE_DASHBOARD') {
            process.env[name] = decrypted_secret;
            vaultSecretsLoaded++;
          }
        });
        const duration = Date.now() - startTime;
        console.log(`✅ Loaded ${vaultSecretsLoaded} secrets from Supabase Vault (${duration}ms)`);
        return;
      }
    } catch (vaultErr) {
      console.warn("Vault unavailable, falling back to app_secrets:", vaultErr.message);
    }
    ...
    const duration = Date.now() - startTime;
    console.log(`✅ Loaded ${legacySecretsLoaded} secrets from legacy app_secrets (${duration}ms)`);
    
  } catch (err) {
    const duration = Date.now() - startTime;
    console.error(`Failed to load secrets from Supabase (${duration}ms):`, err.message);
