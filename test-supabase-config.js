...
async function testSupabaseConnection() {
  ...
  console.log(
    `   SUPABASE_SECRET_KEY: ${
      !!process.env.SUPABASE_SECRET_KEY ? "✅ Configured" : "❌ Missing"
    }`
  );
  console.log(
  ...
  console.log(
    `   SUPABASE_ANON_KEY: ${
      !!process.env.SUPABASE_ANON_KEY ? "✅ Configured" : "❌ Missing"
    }`
  );
  console.log("");
    ...
    if (result.success && result.client) {
      ...
      try {
        ...
        if (tablesError) {
          ...
          console.log(
            `   - enhanced_leads: ${
              tableNames.includes("enhanced_leads") ? "✅" : "❌"
            }`
          );
          console.log(
            `   - campaigns: ${tableNames.includes("campaigns") ? "✅" : "❌"}`
          );
          console.log(
            `   - api_costs: ${tableNames.includes("api_costs") ? "✅" : "❌"}`
          );
        }
      } catch (schemaError) {
        console.log(
          "   Schema check: ⚠️ Limited permissions or missing tables"
        );
      }
    }
