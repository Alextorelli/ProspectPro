...
async function validateWebsite(url: string): Promise<any> {
  if (!url) return { isValid: false, accessible: false, error: 'No website provided' };
...
async function validateEmail(email: string): Promise<any> {
  if (!email) return { isValid: false, deliverable: false, confidence: 0, error: 'No email provided' };
  
  // Basic email format validation
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!emailRegex.test(email)) {
    return { isValid: false, deliverable: false, confidence: 0, error: 'Invalid email format' };
  }
...
function validatePhone(phone: string): any {
  if (!phone) return { isValid: false, format: 'none', error: 'No phone provided' };
...
Deno.serve(async (req) => {
  ...
  try {
    const { leads, skipEmailValidation = false, skipWebsiteCheck = false }: ValidationRequest = await req.json()
    ...
    const validationResults: ValidationResult[] = await Promise.all(
      leads.map(async (lead) => {
        const [websiteValidation, emailValidation, phoneValidation] = await Promise.all([
          skipWebsiteCheck ? { isValid: false, accessible: false, error: 'Skipped' } : validateWebsite(lead.website || ''),
          skipEmailValidation ? { isValid: false, deliverable: false, confidence: 0, error: 'Skipped' } : validateEmail(lead.email || ''),
    ...
    console.log(`Validation complete: ${qualifiedCount}/${leads.length} leads qualified (${response.qualificationRate}%)`)
