...
CREATE POLICY "Users can only see own campaign analytics" ON campaign_analytics FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Users can only see own api usage" ON api_usage_logs FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Users can only see own validation data" ON lead_validation_pipeline FOR ALL USING (auth.uid() = user_id);
