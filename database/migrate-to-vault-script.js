  ...
  try {
    ...
    try {
      ...
      if (appSecretsError) {
        ...
        console.log(
          `📊 Found ${existingSecrets.length} existing secrets in app_secrets`
        );
      }
    } catch (err) {
      console.log(
        "ℹ️  app_secrets table not accessible, creating new vault secrets"
      );
      existingSecrets = [];
    }
    ...
    try {
      ...
      if (vaultError) {
        ...
        console.log(
          `📊 Found ${vaultSecrets.length} existing secrets in vault`
        );
      }
    } catch (err) {
    ...
    for (const secretName of REQUIRED_SECRETS) {
      try {
        ...
        const secretValue =
          existingSecret?.value || "CONFIGURE_IN_SUPABASE_DASHBOARD";
    ...
    if (!DRY_RUN) {
      ...
      console.log(
        "   3. Remove app_secrets table after confirming vault works"
      );
    } else {
      console.log(
