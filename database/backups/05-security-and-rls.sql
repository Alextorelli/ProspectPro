...
END IF;
EXCEPTION
WHEN OTHERS THEN RAISE NOTICE '   - Skipped moving postgis: % (may be non-relocatable in managed environments)',
SQLERRM;
END;
END $$;
...
DO $$
DECLARE rec RECORD;
BEGIN -- Iterate over known function signatures that may exist and alter them safely
FOR rec IN (
  ...
  UNION ALL
  SELECT 'public.calculate_lead_quality_score(jsonb)'
  UNION ALL
  ...
  UNION ALL
  SELECT 'public.get_campaign_analytics(uuid,text)'
  UNION ALL
  SELECT 'public.get_realtime_dashboard_metrics(uuid,text)'
  UNION ALL
  SELECT 'public.leads_within_radius(double precision,double precision,double precision,uuid,text)'
  UNION ALL
  SELECT 'public.search_leads_by_name(text,uuid,integer,text)'
  UNION ALL
  SELECT 'public.update_campaign_statistics(uuid,text)'
  UNION ALL
  SELECT 'public.refresh_analytics_views(text)'
  UNION ALL
  SELECT 'public.archive_old_data(date,text)'
  UNION ALL
  SELECT 'public.validate_rls_security(text)'
  UNION ALL
  SELECT 'public.get_deployment_health_summary(text)'
  UNION ALL
  SELECT 'public.analyze_deployment_failures(integer,text)'
) LOOP -- Check if function by signature exists
IF EXISTS (
  SELECT 1
  FROM pg_proc p
...
EXCEPTION
WHEN OTHERS THEN -- Ignore errors for functions that might not exist or can't be altered
NULL;
END;
END LOOP;
END $$;