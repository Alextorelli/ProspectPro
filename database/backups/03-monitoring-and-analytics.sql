...
CREATE TABLE IF NOT EXISTS railway_webhook_logs (
  ...
  -- Status tracking
  processing_status TEXT DEFAULT 'processed' CHECK (
    processing_status IN ('processed', 'failed', 'retry')
  ),
  error_message TEXT,
...
CREATE TABLE IF NOT EXISTS deployment_metrics (
  ...
  status TEXT NOT NULL CHECK (
    status IN ('success', 'failure', 'crash', 'timeout')
  ),
...
CREATE TABLE IF NOT EXISTS deployment_failures (
  ...
  -- 'dependency', 'syntax', 'memory', 'network', etc.
  -- Failure details
  error_message TEXT,
  build_logs TEXT,
  stack_trace TEXT,
...
END IF;
END $$;
-- Index uses the generated column (immutable)
...
CREATE INDEX IF NOT EXISTS idx_deployment_metrics_build_time ON deployment_metrics(build_time_ms DESC)
WHERE build_time_ms IS NOT NULL;
...
SELECT DATE(processed_at) as deployment_date,
  event_type,
  ...
  COUNT(*) FILTER (
    WHERE event_type = 'deployment.crashed'
  ) as crashed_deployments,
  ROUND(
    COUNT(*) FILTER (
      WHERE event_type = 'deployment.success'
    )::numeric / NULLIF(
      COUNT(*) FILTER (
        WHERE event_type IN ('deployment.success', 'deployment.failed')
      ),
      0
    ) * 100,
    2
  ...
  AVG(
    CASE
      WHEN event_type = 'deployment.success'
...
CREATE OR REPLACE FUNCTION get_deployment_health_summary() RETURNS JSON AS $$
DECLARE result JSON;
BEGIN
    ...
    (
      ...
      SELECT json_agg(
          json_build_object(
            ...
            processed_at,
            'error_details',
            event_data->'deployment'->>'error',
            'build_time_ms',
            CASE
              WHEN (event_data->'deployment'->>'buildTime')::text ~ '^[0-9]+$' THEN (event_data->'deployment'->>'buildTime')::integer
              ELSE NULL
            END
          )
        )
...
CREATE OR REPLACE FUNCTION analyze_deployment_failures(days_back INTEGER DEFAULT 7) RETURNS JSON AS $$
DECLARE result JSON;
BEGIN
SELECT json_build_object(
    'failure_categories',
    (
      ...
      FROM (
          SELECT COALESCE(failure_category, 'uncategorized') as failure_category,
            json_build_object(
              'count',
              COUNT(*),
              'percentage',
              ...
              'avg_time_to_failure_ms',
              ROUND(AVG(build_time_ms)),
              'recent_examples',
              (
                SELECT json_agg(
                    json_build_object(
    ...
    (
      SELECT json_build_object(
          ...
          ROUND(
            EXTRACT(
              EPOCH
              FROM AVG(resolved_at - failure_timestamp)
            ) / 3600,
            1
          ) FILTER (
            WHERE resolved = true
