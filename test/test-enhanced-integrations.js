...
const EnhancedSupabaseClient = require('../modules/api-clients/enhanced-supabase-client');
...
class EnhancedIntegrationTester {
  ...
  async testScrapingDogEnhanced() {
    ...
    if (!process.env.SCRAPINGDOG_API_KEY) {
      this.recordTest('scrapingdog', 'API Key Check', false, 'API key not configured');
      return;
    }
    
    const client = new EnhancedScrapingDogClient(process.env.SCRAPINGDOG_API_KEY, 50);
    ...
    try {
      ...
      this.recordTest('scrapingdog', 'Multi-radius Search', 
      ...
      if (results.length > 0) {
        ...
        this.recordTest('scrapingdog', 'Business Enrichment',
          enriched && enriched.length > 0,
          `Enriched ${enriched.length} businesses`
        );
        
        // Test 3: Email extraction
        const emailsFound = enriched.filter(b => b.emails && b.emails.length > 0).length;
        this.recordTest('scrapingdog', 'Email Extraction',
      ...
      this.recordTest('scrapingdog', 'Cost Tracking',
        costSummary && typeof costSummary.credits_used === 'number',
        `Used ${costSummary.credits_used} credits (~$${costSummary.estimated_cost.toFixed(3)})`
      );
      
    } catch (error) {
      this.recordTest('scrapingdog', 'Multi-radius Search', false, error.message);
    }
  }
  ...
  async testHunterEnhanced() {
    ...
    if (!process.env.HUNTER_IO_API_KEY) {
      this.recordTest('hunter', 'API Key Check', false, 'API key not configured');
      return;
    }
    ...
    try {
      ...
      this.recordTest('hunter', 'Email Pattern Generation',
      ...
      if (process.env.NODE_ENV !== 'production') { // Only in development
        ...
        this.recordTest('hunter', 'Domain Search',
      ...
      this.recordTest('hunter', 'Usage Statistics',
        stats && typeof stats.daily_spend === 'number',
        `Daily spend: $${stats.daily_spend.toFixed(2)}, Success rate: ${stats.search_success_rate}`
      );
  ...
  async testSupabaseEnhanced() {
    ...
    if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SECRET_KEY) {
      this.recordTest('supabase', 'Configuration Check', false, 'Supabase not configured');
      return;
    }
    
    const client = new EnhancedSupabaseClient(
    ...
    try {
      ...
      const testCampaign = await client.createCampaign({
        name: 'Test Campaign - Enhanced Integration',
        search_parameters: { businessType: 'test', location: 'test' },
        budget_limit: 10,
        lead_limit: 5,
        quality_threshold: 70
      }, 'test-user-id');
      
      this.recordTest('supabase', 'Campaign Creation',
        testCampaign && testCampaign.id,
        `Created campaign: ${testCampaign.id}`
      );
      
      // Test 2: Enhanced lead creation
      console.log('  📋 Testing enhanced lead creation...');
      const testLead = {
        ...
        emails: [
          { value: 'test@test.com', verification_status: 'deliverable', confidence: 85 }
        ],
        confidence_score: 85,
        rating: 4.5,
        review_count: 25
      ...
      const savedLead = await client.createEnhancedLead(testLead, testCampaign.id);
      
      this.recordTest('supabase', 'Enhanced Lead Creation',
        savedLead && savedLead.id,
        `Created lead: ${savedLead.id} (${savedLead.confidence_score}% confidence)`
      );
      
      // Test 3: Lead querying with filters
      console.log('  🔍 Testing qualified leads query...');
      const qualifiedLeads = await client.getQualifiedLeads({
        campaignIds: [testCampaign.id],
        minConfidence: 80,
        limit: 10
      });
      
      this.recordTest('supabase', 'Qualified Leads Query',
        qualifiedLeads && Array.isArray(qualifiedLeads),
        `Found ${qualifiedLeads.length} qualified leads`
      );
      
      // Test 4: Campaign analytics
      console.log('  📊 Testing campaign analytics...');
      const analytics = await client.getCampaignAnalytics(testCampaign.id);
      
      this.recordTest('supabase', 'Campaign Analytics',
        analytics && analytics.campaign_id,
        `Analytics: ${analytics.total_leads} leads, $${analytics.total_cost} cost`
      );
  ...
  async testOrchestrator() {
    ...
    const requiredKeys = ['SCRAPINGDOG_API_KEY', 'HUNTER_IO_API_KEY', 'SUPABASE_URL', 'SUPABASE_SECRET_KEY'];
    const missingKeys = requiredKeys.filter(key => !process.env[key]);
    
    if (missingKeys.length > 0) {
      this.recordTest('orchestrator', 'Configuration Check', false, 
    ...
    try {
      ...
      this.recordTest('orchestrator', 'Initialization',
        orchestrator && orchestrator.scrapingDog && orchestrator.hunter && orchestrator.supabase,
      ...
      if (process.env.NODE_ENV !== 'production') {
        ...
        this.recordTest('orchestrator', 'End-to-End Workflow',
        ...
        this.recordTest('orchestrator', 'Cost Control',
          results.stats.total_cost <= 5,
          `Stayed within $5 budget (actual: $${results.stats.total_cost})`
        );
        
        this.recordTest('orchestrator', 'API Integration',
      ...
      this.recordTest('orchestrator', 'End-to-End Workflow', false, error.message);
    }
  }
  ...
  printTestResults() {
    ...
    if (totalFailed === 0) {
      console.log('🎉 ALL TESTS PASSED! Enhanced integration is ready for production.');
    } else if (totalFailed <= 2) {
      console.log('⚠️  Some tests failed, but core functionality is working.');
    } else {
      console.log('❌ Multiple test failures - check configuration and API keys.');
    }
  ...
  static checkConfiguration() {
    ...
    console.log('\nBudget Settings:');
    console.log(`  ScrapingDog Budget: $${process.env.SCRAPINGDOG_MONTHLY_BUDGET || '200'}/month`);
    console.log(`  Hunter.io Budget: $${process.env.HUNTER_MONTHLY_BUDGET || '500'}/month`);
    console.log(`  Daily Limit: $${process.env.DAILY_BUDGET_LIMIT || '25'}/day`);
  }
}
