...
async function testEnhancedBusinessDiscovery() {
    ...
    try {
        ...
        const googlePlaces = new GooglePlacesClient(process.env.GOOGLE_PLACES_API_KEY);
        ...
        for (let i = 0; i < testBusinesses.length; i++) {
            ...
            if (business.name && business.address) {
                try {
                    ...
                    const stateValidation = await enhancedStateRegistry.searchBusinessAcrossStates(
                        business.name,
                        business.address,
                        'California'
                    );
                    ...
                    console.log(`      - Confidence Score: ${stateValidation.confidenceScore}%`);
                    console.log(`      - APIs Queried: ${stateValidation.qualityMetrics?.totalAPIsQueried || 7}`);
                    ...
                    if (stateValidation.confidenceScore > 50) {
                        ...
                        business.stateRegistryValidation = {
                            ...
                            sourcesChecked: stateValidation.qualityMetrics?.totalAPIsQueried || 7
                        };
                    } else {
            ...
            if (business.email) {
                try {
                    ...
                    const emailValidation = await zeroBounce.validateEmail(business.email);
                    enhancedValidationResults.emailValidations++;
                    enhancedValidationResults.totalEnhancedCost += emailValidation.cost || 0.007;
                    
                    console.log(`      - Email Status: ${emailValidation.status}`);
                    console.log(`      - Is Valid: ${emailValidation.isValid}`);
                    console.log(`      - Confidence: ${emailValidation.confidence || 'N/A'}%`);
        ...
        console.log(`   - State Registry Checks: ${enhancedValidationResults.stateRegistryChecks}`);
        console.log(`   - State Registry Matches: ${enhancedValidationResults.stateRegistryMatches}`);
        console.log(`   - Email Validations: ${enhancedValidationResults.emailValidations}`);
        console.log(`   - Deliverable Emails: ${enhancedValidationResults.emailDeliverable}`);
        console.log(`   - Total Enhanced Cost: $${enhancedValidationResults.totalEnhancedCost.toFixed(3)}`);

        if (enhancedValidationResults.stateRegistryChecks > 0 || enhancedValidationResults.emailValidations > 0) {
