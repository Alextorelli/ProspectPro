...
async function testBusinessDiscoveryIntegration() {
    ...
    try {
        ...
        console.log(`   Budget: $${testQuery.budgetLimit}, Quality Threshold: ${testQuery.qualityThreshold}%`);

        const startTime = Date.now();
        
        // Make request to business discovery endpoint
        const response = await axios.post(`${baseURL}/api/business/discover`, testQuery, {
        ...
        if (response.status === 200 && response.data.success) {
            ...
            if (dataEnhancements.stateRegistryValidations) {
                console.log(`   - Total Checked: ${dataEnhancements.stateRegistryValidations.totalChecked}`);
                console.log(`   - Validated Businesses: ${dataEnhancements.stateRegistryValidations.validatedBusinesses}`);
                console.log(`   - Validation Rate: ${dataEnhancements.stateRegistryValidations.validationRate}%`);
            } else {
                console.log('   - No state registry validations found (may need API endpoint fixes)');
            }
            
            console.log('\n4. 📧 Advanced Email Validations:');
            if (dataEnhancements.advancedEmailValidations) {
                console.log(`   - Total Validated: ${dataEnhancements.advancedEmailValidations.totalValidated}`);
                console.log(`   - Deliverable Emails: ${dataEnhancements.advancedEmailValidations.deliverableEmails}`);
                console.log(`   - Deliverability Rate: ${dataEnhancements.advancedEmailValidations.deliverabilityRate}%`);
            } else {
            ...
            if (dataEnhancements.qualityImprovements) {
                console.log(`   - Leads Enhanced: ${dataEnhancements.qualityImprovements.leadsEnhanced}`);
                console.log(`   - Enhancement Cost: $${dataEnhancements.qualityImprovements.enhancedValidationCost}`);
                console.log(`   - Government APIs Used: ${dataEnhancements.qualityImprovements.governmentAPIsSources}`);
                console.log(`   - Free API Calls: ${dataEnhancements.qualityImprovements.totalFreeAPIsUsed}`);
            }
            ...
            if (results && results.length > 0) {
                ...
                console.log(`   Confidence Score: ${sampleLead.confidenceScore || 'N/A'}%`);
                
                if (sampleLead.stateRegistryValidation) {
                    console.log('   State Registry: ✅ Validated');
                    console.log(`   Registry Confidence: ${sampleLead.stateRegistryValidation.confidenceScore}%`);
                    console.log(`   Sources Checked: ${sampleLead.stateRegistryValidation.sourcesChecked}`);
                } else {
                ...
                if (sampleLead.emailValidation) {
                    ...
                    console.log(`   Email Deliverable: ${sampleLead.emailValidation.deliverable ? '✅' : '❌'}`);
                } else {
