...
const EnhancedSupabaseClient = require('./api-clients/enhanced-supabase-client');
...
class EnhancedLeadDiscoveryOrchestrator {
  constructor(config = {}) {
    ...
    this.supabase = new EnhancedSupabaseClient(
    ...
    // Configuration
    this.config = {
      minConfidenceScore: config.minConfidenceScore || process.env.MIN_CONFIDENCE_SCORE || 70,
      maxLeadsPerCampaign: config.maxLeadsPerCampaign || process.env.MAX_LEADS_PER_CAMPAIGN || 100,
  ...
  async runEnhancedLeadDiscovery(searchParams, userId) {
    ...
    try {
      ...
      const businessesWithEmails = await this.discoverEmails(enrichedBusinesses);
      console.log(`✅ Found emails for ${businessesWithEmails.filter(b => b.emails?.length > 0).length} businesses`);
      ...
      const savedLeads = await this.saveEnhancedLeads(qualifiedLeads, campaign.id);
      console.log(`✅ Saved ${savedLeads.length} leads to database`);
      
      // Step 7: Generate comprehensive results
      const results = await this.generateCampaignResults(campaign.id, startTime);
  ...
  async createCampaign(searchParams, userId) {
    const campaignData = {
      ...
      quality_threshold: searchParams.qualityThreshold || this.config.minConfidenceScore
    };
  ...
  async enrichBusinesses(businesses) {
    ...
    const worthEnriching = businesses.filter(business => 
      business.confidence_score >= 50 && // Basic quality threshold
      business.website && 
      this.scrapingDog.isValidWebsite(business.website)
    );
    
    console.log(`📊 Selected ${worthEnriching.length} businesses for enrichment`);
    ...
    try {
      const enriched = await this.scrapingDog.enrichBusinessData(worthEnriching);
      ...
      const enrichedMap = new Map(enriched.map(b => [b.data_id || b.title, b]));
      
      return businesses.map(business => {
        const enrichedVersion = enrichedMap.get(business.data_id || business.title);
  ...
  async discoverEmails(businesses) {
    ...
    const businessesWithDomains = businesses.filter(business => 
      business.website && 
      !this.hunter.isInvalidDomain(this.hunter.extractDomain(business.website))
    );
    
    console.log(`🎯 Selected ${businessesWithDomains.length} businesses for email discovery`);
    ...
    for (let i = 0; i < businessesWithDomains.length; i += batchSize) {
      ...
      console.log(`📦 Processing email batch ${Math.floor(i/batchSize) + 1}/${Math.ceil(businessesWithDomains.length/batchSize)}`);
      ...
      try {
        ...
        // Track costs
        const batchCost = batchResults.reduce((sum, result) => sum + (result.cost || 0), 0);
        this.campaignStats.total_cost += batchCost;
        this.campaignStats.api_calls.hunter += batch.length;
        
      } catch (error) {
        console.error(`❌ Email batch ${Math.floor(i/batchSize) + 1} failed:`, error);
        // Continue with remaining batches
      }
      
      // Check budget limits
      if (this.campaignStats.total_cost > 50) { // $50 safety limit
    ...
    return businesses.map(business => {
      const emailResult = emailMap.get(business.business_name || business.title);
  ...
  async validateAndScore(businesses) {
    ...
    const scoredBusinesses = businesses.map(business => {
      ...
      return {
        ...
        enrichment_cost: (business.discovery_cost || 0) + (business.email_discovery_cost || 0)
      };
    });
    
    // Filter qualified leads
    const qualifiedLeads = scoredBusinesses.filter(business => business.is_qualified);
    
    this.campaignStats.leads_qualified = qualifiedLeads.length;
    
    console.log(`✅ ${qualifiedLeads.length}/${businesses.length} businesses qualify (${this.config.minConfidenceScore}%+ confidence)`);
  ...
  async saveEnhancedLeads(leads, campaignId) {
    ...
    for (const lead of leads) {
      try {
        ...
        // Save to database
        const savedLead = await this.supabase.createEnhancedLead(leadData, campaignId);
    ...
    console.log(`✅ Successfully saved ${savedLeads.length}/${leads.length} leads`);
    return savedLeads;
  }
  ...
  async generateCampaignResults(campaignId, startTime) {
    ...
    return {
      ...
      stats: {
        ...
        cost_per_lead: this.campaignStats.leads_qualified > 0 ? 
          (this.campaignStats.total_cost / this.campaignStats.leads_qualified).toFixed(4) : 0,
        success_rate: this.campaignStats.leads_discovered > 0 ?
          Math.round((this.campaignStats.leads_qualified / this.campaignStats.leads_discovered) * 100) : 0
      },
      analytics: analytics,
      api_usage: {
        scrapingdog: this.scrapingDog.getCostSummary(),
        hunter: this.hunter.getUsageStats(),
        total_api_calls: Object.values(this.campaignStats.api_calls).reduce((a, b) => a + b, 0)
      },
  ...
  calculateEnhancedConfidenceScore(business) {
    ...
    if (business.emails && business.emails.length > 0) {
      ...
      const verifiedEmails = business.emails.filter(e => e.verification_status === 'deliverable');
    ...
    if (business.social_links && Object.keys(business.social_links).length > 0) score += 5;
    if (business.website_content && business.website_content.length > 1000) score += 5;
  ...
  generateRecommendations() {
    ...
    if (this.campaignStats.total_cost > 25) {
      recommendations.push({
        type: 'budget',
        message: 'Consider reducing search radius or quality threshold to lower costs',
        priority: 'medium'
      });
    }
    
    // Quality optimization
    const qualificationRate = this.campaignStats.leads_discovered > 0 ?
      (this.campaignStats.leads_qualified / this.campaignStats.leads_discovered) : 0;
    
    if (qualificationRate < 0.3) {
      recommendations.push({
        type: 'quality',
        message: 'Low qualification rate - consider refining search terms or location',
        priority: 'high'
      });
    }
    
    // Email discovery
    const emailCostPerLead = this.hunter.getUsageStats().daily_spend / this.campaignStats.leads_qualified;
    if (emailCostPerLead > 0.20) {
      recommendations.push({
        type: 'email',
        message: 'Email discovery costs are high - consider using pattern generation first',
        priority: 'low'
      });
    }
