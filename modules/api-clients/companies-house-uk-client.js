...
class CompaniesHouseUKClient {
    constructor(apiKey = null) {
        ...
        this.costPerRequest = 0.00; // Free API with registration
        ...
        if (!this.apiKey) {
            console.warn('⚠️ Companies House UK API key not found. Set COMPANIES_HOUSE_UK_API_KEY environment variable.');
        }
    }
    ...
    async searchCompanies(searchTerm, options = {}) {
        if (!this.apiKey) {
            console.warn('⚠️ Companies House UK API key not configured, returning mock response');
        ...
        // Check cache first
        const cacheKey = `ch_uk_search_${searchTerm.toLowerCase().trim()}_${JSON.stringify(options)}`;
        ...
        try {
            ...
            const response = await this.makeRequest(`/search/companies?${searchParams}`);
    ...
    async getCompanyByNumber(companyNumber, options = {}) {
        ...
        try {
            ...
            const additionalData = await Promise.allSettled([
                ...
                options.includePSC ? this.getPersonsOfSignificantControl(cleanNumber) : null,
            ...
            const normalizedResponse = this.normalizeCompanyResponse(enrichedData, cleanNumber);
    ...
    async getCompanyOfficers(companyNumber) {
        try {
            ...
            console.warn(`Failed to get officers for ${companyNumber}:`, error.message);
            return null;
        }
    }
    ...
    async getPersonsOfSignificantControl(companyNumber) {
        try {
            const response = await this.makeRequest(`/company/${companyNumber}/persons-with-significant-control`);
    ...
    async getCompanyCharges(companyNumber) {
        try {
            const response = await this.makeRequest(`/company/${companyNumber}/charges`);
            return this.normalizeChargesResponse(response);
        } catch (error) {
            console.warn(`Failed to get charges for ${companyNumber}:`, error.message);
            return null;
        }
    }
            ...
            try {
                ...
                if (response.status === 401) {
                    ...
                    if (attempt < retries) {
                        ...
                        console.log(`Companies House rate limit hit. Waiting ${delay}ms before retry ${attempt}/${retries}`);
                        await new Promise(resolve => setTimeout(resolve, delay));
                        continue;
                    }
                    throw new Error('Companies House rate limit exceeded - Please try again later');
                } else if (response.status >= 500) {
                    if (attempt < retries) {
                        const delay = Math.pow(2, attempt) * 1000;
                        console.log(`Companies House server error ${response.status}. Retrying in ${delay}ms`);
                        await new Promise(resolve => setTimeout(resolve, delay));
                        continue;
                    }
    ...
    async waitForRateLimit() {
        ...
        if (this.requestTimes.length >= this.rateLimitPer5Min) {
            ...
            if (waitTime > 0) {
                console.log(`Waiting ${waitTime}ms for Companies House rate limit reset`);
    ...
    async enhanceWithCompanyDetails(companies, includeOfficers = false) {
        ...
        for (const company of companies) {
            try {
                ...
                const details = await this.getCompanyByNumber(company.companyNumber, detailsOptions);
                ...
                console.warn(`Failed to get details for ${company.companyNumber}:`, error.message);
    ...
    normalizeSearchResponse(data, searchTerm, options) {
        ...
        return {
            ...
            companies: companies.map(company => ({
                ...
                companyStatusDescription: this.companyStatuses[company.company_status] || company.company_status,
                companyType: company.company_type || null,
                companyTypeDescription: this.companyTypes[company.company_type] || company.company_type,
    ...
    normalizeCompanyResponse(data, companyNumber) {
        ...
        return {
            ...
            companyDetails: {
                ...
                // Address
                registeredOfficeAddress: this.normalizeAddress(company.registered_office_address),
                ...
                confirmationStatementDate: company.confirmation_statement?.next_due || null,
    ...
    getMockResponse(searchTerm) {
        return {
            ...
            totalResults: 0,
            companies: [],
            error: 'API key not configured - this would search Companies House UK database',
