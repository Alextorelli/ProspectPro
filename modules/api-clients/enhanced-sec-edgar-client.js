...
class EnhancedSECEdgarClient {
    constructor() {
        ...
        this.companyCIKUrl = 'https://www.sec.gov/files/company_tickers_exchange.json';
        ...
        this.userAgent = 'ProspectPro Business Intelligence System (support@prospectpro.ai)';
    }
    ...
    async searchCompanies(searchTerm, options = {}) {
        ...
        try {
            ...
            const response = this.normalizeSearchResponse(enrichedMatches, searchTerm);
    ...
    async getCompanyFacts(cik) {
        ...
        try {
            const data = await this.makeRequest(`/api/xbrl/companyfacts/CIK${paddedCIK}.json`);
    ...
    async getLatestFilings(cik, limit = 5) {
        ...
        try {
            const data = await this.makeRequest(`/api/xbrl/submissions/CIK${paddedCIK}.json`);
    ...
    async loadCompaniesDirectory() {
        ...
        if (this.companiesCache && this.companiesCacheExpiry && now < this.companiesCacheExpiry) {
            return this.companiesCache;
        }

        try {
            // Load company tickers
            const tickerData = await this.makeRequest('/files/company_tickers.json', true);
            
            // Load exchange data if available
            let exchangeData = null;
            try {
                exchangeData = await this.makeRequest('/files/company_tickers_exchange.json', true);
            } catch (e) {
                console.log('Exchange data not available, using ticker data only');
            }

            // Normalize and combine data
            this.companiesCache = this.normalizeCompaniesDirectory(tickerData, exchangeData);
            this.companiesCacheExpiry = now + (24 * 60 * 60 * 1000); // Cache for 24 hours
            
            console.log(`Loaded ${Object.keys(this.companiesCache).length} companies from SEC directory`);
            return this.companiesCache;
    ...
    findCompanyMatches(searchTerm) {
        ...
        for (const [key, company] of Object.entries(this.companiesCache)) {
            ...
            // Exact CIK match
            else if (company.cik.toString() === searchTerm || 
                     company.cik.toString().padStart(10, '0') === searchTerm.padStart(10, '0')) {
                matchScore = 95;
                matchType = 'cik_exact';
            }
            ...
            try {
                ...
                if (response.status === 429) {
                    ...
                    if (attempt < retries) {
                        ...
                        console.log(`SEC rate limit hit. Waiting ${delay}ms before retry ${attempt}/${retries}`);
                    ...
                    if (attempt < retries) {
                        ...
                        console.log(`SEC server error ${response.status}. Retrying in ${delay}ms`);
                        await new Promise(resolve => setTimeout(resolve, delay));
                        continue;
                    }
    ...
    normalizeCompanyFacts(factsData) {
        ...
        return {
            ...
            businessDescription: dei.EntityBusinessDescription?.units?.USD?.[0]?.val || null,
            ...
            // Entity information
            entityInfo: {
                fiscalYearEnd: dei.EntityFiscalYearEndMonthOfYear?.units?.USD?.[0]?.val || null,
                commonStockShares: dei.EntityCommonStockSharesOutstanding?.units?.shares?.[0]?.val || null,
    ...
    extractLatestFinancialData(usGaap) {
        ...
        keyMetrics.forEach(metric => {
            if (usGaap[metric] && usGaap[metric].units && usGaap[metric].units.USD) {
                const latestValue = usGaap[metric].units.USD
    ...
    getUsageStats() {
        return {
            ...
            rateLimitStatus: {
                ...
                nextAvailableTime: this.requestTimes.length >= this.rateLimitPerSecond ? 
                    new Date(Math.min(...this.requestTimes) + 1000).toISOString() : 'now'
            },
            cacheStats: {
            ...
            apiInfo: {
                ...
                companiesLoaded: this.companiesCache ? Object.keys(this.companiesCache).length : 0
            }
        };
    }
