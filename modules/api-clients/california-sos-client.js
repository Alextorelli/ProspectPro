...
class CaliforniaSOSClient {
    constructor(apiKey = null) {
        ...
        this.baseUrl = process.env.CALIFORNIA_SOS_BASE_URL || 'https://calico.sos.ca.gov/cbc/v1/api';
        ...
        if (!this.apiKey) {
            console.warn('⚠️ California SOS API key not found. Set CALIFORNIA_SOS_API_KEY environment variable.');
        }
    }
    ...
    generateCacheKey(searchTerm, options = {}) {
        return `ca_sos_${searchTerm.toLowerCase().trim()}_${JSON.stringify(options)}`;
    }
    ...
    async searchByKeyword(businessName, options = {}) {
        if (!this.apiKey) {
            console.warn('⚠️ California SOS API key not configured, returning mock response');
        ...
        if (!this.checkRateLimit()) {
            throw new Error('California SOS rate limit exceeded. Please try again later.');
        }
        ...
        try {
            ...
            const response = await this.makeRequest(`/BusinessEntityKeywordSearch?${searchParams}`);
            const normalizedResponse = this.normalizeSearchResponse(response, businessName);
    ...
    async getEntityDetails(entityNumber) {
        ...
        try {
            const response = await this.makeRequest(`/BusinessEntityDetails?entity-number=${encodeURIComponent(entityNumber)}`);
            const normalizedResponse = this.normalizeEntityResponse(response, entityNumber);
            ...
            try {
                ...
                if (response.status === 400) {
                    ...
                    if (attempt < retries) {
                        ...
                        console.log(`Rate limit exceeded. Waiting ${delay}ms before retry ${attempt}/${retries}`);
    ...
    normalizeSearchResponse(data, searchTerm) {
        ...
        entities.forEach(entity => {
            if (entity.EntityName && entity.EntityName.toLowerCase() === searchTerm.toLowerCase()) {
                exactMatches++;
            }
        });
        ...
        return {
            ...
            entities: entities.map(entity => ({
                ...
                registrationDate: entity.RegistrationDate || entity.FormationDate || null,
    ...
    getMockResponse(businessName) {
        return {
            ...
            totalResults: 0,
            entities: [],
            error: 'API key not configured - this would search California SOS database',
    ...
    getUsageStats() {
        return {
            ...
            rateLimitStatus: {
                ...
                resetTime: new Date(this.lastResetTime + this.rateLimitWindow).toISOString()
            },
            cacheStats: {
