...
class EnhancedStateRegistryClient {
    constructor() {
        ...
        this.apis = {
            californiaSOS: {
                ...
                cost: 0.00,
                documentation: 'https://calicodev.sos.ca.gov/content/California%20SOS%20BE%20Public%20Search%20API%20Guide%20v1.0.4.pdf'
            },
            newYorkSOS: {
                ...
                enabled: true,
                rateLimit: 1000, // requests per hour without token
                qualityScore: 75,
                integrationStage: 'Stage 3: Validation',
                cost: 0.00,
            ...
            newYorkTaxParcels: {
                ...
                cost: 0.00,
                documentation: 'https://data.gis.ny.gov/maps/8af5cef967f8474a9f262684b8908737'
            },
            connecticutUCC: {
            ...
            uspto: {
                ...
                apiKey: process.env.USPTO_API_KEY, // Free with registration
            ...
            courtListener: {
                ...
                token: process.env.COURTLISTENER_TOKEN, // Free with registration
                enabled: true, // Works without token but with limits
                rateLimit: 5000, // requests per hour for authenticated users
                qualityScore: 60,
                integrationStage: 'Stage 3: Validation',
                cost: 0.00,
    ...
    async searchBusinessAcrossStates(businessName, address = null, state = null) {
        ...
        results.validationResults.uspto = this.processSettledResult(enhancedValidation[address ? 1 : 0], 'USPTO Trademarks');
        results.validationResults.secEdgar = this.processSettledResult(enhancedValidation[address ? 2 : 1], 'SEC EDGAR');
        results.qualityMetrics.totalAPIsQueried += 2;
        ...
        results.qualityMetrics.totalCost = 0.00; // All free APIs
    ...
    async searchCaliforniaSOS(businessName) {
        ...
        try {
            ...
            return {
                ...
                businesses: data.EntityList?.map(entity => ({
    ...
    async searchNYTaxParcels(address) {
        ...
        try {
            ...
            if (!geocodeData.result?.addressMatches || geocodeData.result.addressMatches.length === 0) {
                return { found: false, error: 'Address not found in geocoding service' };
            }
            ...
            return {
                ...
                properties: parcelData.features?.map(feature => ({
    ...
    async searchSECFilings(businessName) {
        ...
        try {
            ...
            const response = await fetch(`${this.apis.secEdgar.baseUrl}/api/xbrl/frames/us-gaap/EntityCommonStockSharesOutstanding/USD/CY2023Q4I.json`, {
                headers: {
    ...
    async searchUSPTOTrademarks(businessName) {
        if (!this.apis.uspto.enabled) {
            return { found: false, note: 'USPTO API key not configured', source: 'USPTO', setupRequired: true };
        }
        ...
        try {
            ...
            return {
                ...
                trademarks: data.response?.docs?.map(doc => ({
                ...
                apiCost: 0.00, // Free with API key
                qualityScore: 60
            };
    ...
    async searchCourtListener(businessName) {
        ...
        try {
            ...
            return {
                ...
                legalCases: data.results?.map(case_ => ({
                    id: case_.id,
                ...
                riskIndicators: {
                    ...
                    recentActivity: data.results?.some(c => 
    ...
    getAPIStatus() {
        const status = {
            overview: {
                ...
                enabledAPIs: Object.values(this.apis).filter(api => api.enabled).length,
                totalCost: 0.00,
                averageQualityScore: Math.round(
                    Object.values(this.apis).reduce((sum, api) => sum + api.qualityScore, 0) / 
        ...
        Object.entries(this.apis).forEach(([apiName, config]) => {
            ...
            const now = Date.now();
            const hourlyRequests = recentRequests.filter(timestamp => now - timestamp < 3600000);
            ...
            status.apis[apiName] = {
                ...
                quotaResetTime: hourlyRequests.length > 0 ? 
    ...
    async searchStateRegistries(businessName, businessAddress) {
        ...
        const result = await this.searchBusinessAcrossStates(businessName, businessAddress);
        ...
        return {
            ...
            incorporationState: result.registrationDetails.registeredStates?.[0] || null,
