...
class MonitoringEnabledLeadDiscovery {
  ...
  async discoverLeadsWithMonitoring(campaignConfig) {
    ...
    try {
      console.log(
        `🚀 Starting monitored lead discovery for campaign: ${campaignId}`
      );
      ...
      const businesses = await this.discoverBusinessesWithMetrics(
        campaignId,
        campaignConfig
      );
      ...
      const enrichedBusinesses = await this.enrichEmailsWithMetrics(
        campaignId,
        businesses
      );
      ...
      const qualifiedLeads = await this.validateLeadsWithMetrics(
        campaignId,
        enrichedBusinesses
      );
      ...
      console.log(
        `🎉 Monitored lead discovery completed: ${qualifiedLeads.length} qualified leads in ${totalTime}ms`
      );
  ...
  async discoverBusinessesWithMetrics(campaignId, config) {
    ...
    try {
      ...
      const businesses =
        await this.scrapingDogClient.searchBusinessesMultiRadius(
          config.businessType,
          config.location,
      ...
      console.log(
        `  ✅ Found ${businesses.length} businesses via ScrapingDog in ${responseTime}ms`
      );
      ...
      console.error(
        `  ❌ ScrapingDog discovery failed in ${responseTime}ms:`,
        error.message
      );
      throw error;
    }
    ...
    for (const business of businesses) {
      ...
      try {
        ...
        const emailPatterns = await this.hunterClient.generateEmailPatterns(
          ...
          business.website ||
            business.business_name.replace(/\s+/g, "").toLowerCase() + ".com"
        );
        ...
        console.log(
          `  📧 Generated ${emailPatterns.length} email patterns for ${business.business_name}`
        );
      } catch (error) {
        const responseTime = Date.now() - startTime;
        ...
        console.error(
          `  ❌ Email enrichment failed for ${business.business_name}:`,
          error.message
        );
      }
    }
  ...
  async validateLeadsWithMetrics(campaignId, businesses) {
    ...
    for (const business of businesses) {
      ...
      if (qualityScore >= 80) {
        ...
        await this.metricsClient.trackLeadDiscovery(campaignId, qualifiedLead, {
          scrapingdog: {
            amount: 0.01,
            success: true,
            responseTime: 500,
        ...
        console.log(
          `  ✅ Qualified: ${business.business_name} (${qualityScore}% confidence)`
        );
      } else {
        console.log(
          `  ⚠️  Below threshold: ${business.business_name} (${qualityScore}% confidence)`
        );
      }
    }
  ...
  setupDashboardMonitoring(callback) {
    ...
    const subscriptions = this.metricsClient.setupRealTimeSubscriptions(
      (eventType, data) => {
...
async function runMonitoredCampaign() {
  ...
  const subscriptions = monitoredDiscovery.setupDashboardMonitoring(
    (eventType, data) => {
  ...
  const results = await monitoredDiscovery.discoverLeadsWithMonitoring(
    campaignConfig
  );
  ...
  const analytics = await monitoredDiscovery.getCampaignAnalytics(
    campaignConfig.id
  );
