...
class SecurityHardening {
  ...
  getMiddleware() {
    ...
    middleware.push(helmet({
      contentSecurityPolicy: {
        directives: {
          ...
          mediaSrc: ["'self'"],
          frameSrc: ["'none'"],
          upgradeInsecureRequests: this.securityConfig.isProduction ? [] : null
        },
      },
    ...
    middleware.push(cors({
      origin: (origin, callback) => {
        ...
        if (!origin && !this.securityConfig.isProduction) {
    ...
    if (!this.rateLimiters.has('api')) {
      ...
      const middleware = async (req, res, next) => {
        ...
        try {
          ...
          console.warn(`🚫 API rate limit exceeded for ${req.ip} (${req.user?.id || 'anonymous'})`);
    ...
    if (!this.rateLimiters.has('expensive')) {
      ...
      const middleware = async (req, res, next) => {
        ...
        try {
          ...
          console.warn(`🚫 Expensive operation rate limit exceeded for ${req.ip} (${req.user?.id || 'anonymous'})`);
          console.warn(`   Endpoint: ${req.method} ${req.path}`);
          
          res.status(429).json({
            error: 'Expensive operation rate limit exceeded',
            message: 'Too many expensive operations from this IP. These operations consume external API quota.',
            retryAfter: `${remainingMin} minutes`,
            limit: 10,
            window: '1 hour'
          });
    ...
    return {
      validateCampaignInput: (req, res, next) => {
        ...
        if (budgetLimit !== undefined) {
          if (typeof budgetLimit !== 'number' || budgetLimit < 0 || budgetLimit > 1000) {
  ...
  getSecurityLogger() {
    return (req, res, next) => {
      ...
      if (req.headers['user-agent']?.includes('bot') && !req.path.startsWith('/health')) {
      ...
      res.on('finish', () => {
        ...
        if (securityEvents.length > 0) {
          console.warn(`🚨 Security events for ${req.ip}: ${securityEvents.join(', ')}`);
        }
      });
  ...
  getSecurityStatus() {
    return {
      ...
      authConfigured: !!(this.securityConfig.adminPassword || this.securityConfig.personalAccessToken)
    };
  }
