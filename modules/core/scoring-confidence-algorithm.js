...
class EnhancedConfidenceScoring {
  constructor() {
    ...
    this.governmentAPIs = {
      ...
      nonprofit: { weight: 0.6, maxBoost: 15, priority: "MEDIUM" },
      companiesHouseUK: { weight: 0.7, maxBoost: 20, priority: "MEDIUM" },
    };
    ...
    this.sectorAdjustments = {
      "Public Company": {
        multiplier: 1.15,
        bonus: 15,
        description: "SEC registered companies have enhanced credibility",
      },
      Nonprofit: {
        multiplier: 1.1,
        bonus: 12,
        description: "IRS registered nonprofits have good credibility",
      },
      "UK Company": {
        multiplier: 1.08,
        bonus: 10,
        description:
          "Companies House registered businesses have good credibility",
      },
      "State Registered Business": {
        multiplier: 1.05,
        bonus: 8,
        description: "State registered businesses have basic credibility",
      },
      Unknown: {
        multiplier: 1.0,
        bonus: 0,
        description: "No sector classification available",
      },
    };
  ...
  calculateConfidenceScore(businessData) {
    ...
    const preRiskScore =
      baseScore +
      governmentBoost.totalBoost +
      multiSourceBonus +
      sectorAdjustment.bonus;
    ...
    console.log(
      `📊 Confidence scoring complete: ${Math.round(
        finalScore
      )}% (${qualityTier})`
    );
  ...
  calculateGovernmentAPIBoost(businessData) {
    ...
    Object.keys(this.governmentAPIs).forEach((apiKey) => {
      ...
      if (apiResult?.found && apiResult.totalResults > 0) {
        ...
        const weightedBoost = Math.min(
          rawBoost * apiConfig.weight,
          apiConfig.maxBoost
        );
        ...
        boost.details.push(
          `${apiKey}: +${Math.round(weightedBoost)} (${
            apiResult.totalResults
          } matches)`
        );
      } else {
  ...
  calculateMultiSourceBonus(businessData) {
    ...
    const sourcesWithMatches = Object.keys(this.governmentAPIs).filter(
      (apiKey) => {
  ...
  calculateSectorAdjustment(businessData, baseScore) {
    ...
    const adjustment =
      this.sectorAdjustments[sectorType] || this.sectorAdjustments["Unknown"];
  ...
  getRecommendedActions(businessData, score) {
    ...
    if (score < this.qualityThresholds.acceptable) {
      ...
      if (
        ...
        !businessData.websiteValidation?.accessible
      ) {
  ...
  scoreAddress(data) {
    ...
    const hasStreetType =
      /\b(st|street|ave|avenue|rd|road|blvd|boulevard|dr|drive|ln|lane|ct|court|way|pl|place)\b/.test(
        address
      );
  ...
  scoreRegistration(data) {
    ...
    if (data.registryValidation?.california?.found)
      registrationSources.push("CA");
  ...
  generateScoringAnalytics(scoredBusinesses) {
    const analytics = {
      ...
      governmentValidation: {
        hasAnyMatch: scoredBusinesses.filter(
          (b) => b.governmentValidation?.hasAnyMatch
        ).length,
        averageBoost: 0,
        sourceDistribution: {},
      },
    ...
    // Calculate averages
    const totalScore = scoredBusinesses.reduce(
      (sum, b) => sum + b.finalConfidenceScore,
      0
    );
    ...
    scoredBusinesses.forEach((business) => {
      ...
      analytics.qualityTierDistribution[tier] =
        (analytics.qualityTierDistribution[tier] || 0) + 1;
    ...
    const govValidatedBusinesses = scoredBusinesses.filter(
      (b) => b.governmentValidation?.hasAnyMatch
    );
    if (govValidatedBusinesses.length > 0) {
      const totalGovBoost = govValidatedBusinesses.reduce(
        (sum, b) =>
          sum + (b.confidenceScoring?.governmentAPIBoost?.totalBoost || 0),
        0
      );
      analytics.governmentValidation.averageBoost = Math.round(
        totalGovBoost / govValidatedBusinesses.length
      );
    }
