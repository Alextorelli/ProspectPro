...
class EnhancedLeadDiscoveryWithGovernmentAPIs {
  constructor(apiKeys = {}) {
    ...
    this.companiesHouseClient = new CompaniesHouseUKClient(apiKeys.companiesHouseUK);
    ...
    this.governmentAPIWeights = {
      ...
      nonprofit: 0.60,        // 60% quality score
      companiesHouseUK: 0.70  // 70% quality score
    };
    
    console.log('🔧 Enhanced Lead Discovery Algorithm with Phase 1 Government APIs initialized');
    console.log('🏛️ Government APIs: California SOS, SEC EDGAR, ProPublica Nonprofit, Companies House UK');
  }
  ...
  async discoverAndValidateLeads(businesses, options = {}) {
    ...
    console.log(`🚀 Starting enhanced lead discovery with government APIs for ${businesses.length} businesses`);
    console.log(`💰 Budget limit: $${budgetLimit}, Quality threshold: ${qualityThreshold}%`);
    console.log(`🏛️ Government APIs enabled: ${enableGovernmentAPIs ? 'YES' : 'NO'}`);
    ...
    for (const business of businesses.slice(0, maxResults)) {
      try {
        ...
        const enhancedBusiness = await this.processBusinessThroughEnhancedPipeline(
          business, 
          options,
    ...
    console.log(`🏛️ Government API validation rate: ${((governmentValidatedCount/processedCount)*100).toFixed(1)}%`);
    console.log(`💰 Total cost: $${this.totalCost.toFixed(2)} (Government APIs: $0.00)`);
   ...
   */
  async processBusinessThroughEnhancedPipeline(business, options, enableGovernmentAPIs, governmentAPIOptions) {
    // Stage 1: Discovery + Pre-validation + Government Registry Validation
    const stage1Result = await this.stage1_EnhancedDiscoveryAndPreValidation(business, enableGovernmentAPIs, governmentAPIOptions);
    ...
    if (stage1Result.preValidationScore < 50 && !stage1Result.governmentValidation?.hasAnyMatch) {
    ...
    // Stage 2: Enrichment + Property Intelligence + Business Intelligence  
    const stage2Result = await this.stage2_EnhancedEnrichmentAndIntelligence(stage1Result, governmentAPIOptions);
    
    // Stage 3: Validation + Risk Assessment + Sector Classification
    const stage3Result = await this.stage3_EnhancedValidationAndRiskAssessment(stage2Result);
    
    // Stage 4: Quality Scoring + Confidence Boosting + Export Preparation
    const finalResult = await this.stage4_EnhancedQualityScoringAndExport(stage3Result);
   ...
   */
  async stage1_EnhancedDiscoveryAndPreValidation(business, enableGovernmentAPIs, governmentAPIOptions) {
    ...
    if (preValidationScore >= 40 || enableGovernmentAPIs) { // Lower threshold for government validation
      ...
      if (enableGovernmentAPIs) {
        governmentValidation = await this.validateThroughGovernmentAPIs(business, governmentAPIOptions);
      }
    }
   ...
   */
  async stage2_EnhancedEnrichmentAndIntelligence(businessData, governmentAPIOptions) {
    ...
    if (businessData.governmentValidation?.hasAnyMatch) {
      businessIntelligence = await this.getBusinessIntelligenceFromGovernmentAPIs(
        businessData, 
        governmentAPIOptions
      );
    }
    ...
    if (shouldDiscoverEmail && this.hunterClient) {
      try {
        ...
        console.warn(`Email discovery failed for ${businessData.name}:`, error.message);
      }
    }
  ...
  async stage3_EnhancedValidationAndRiskAssessment(businessData) {
    ...
    if (businessData.emailDiscovery?.emails?.length && this.neverBounceClient) {
      try {
        emailValidation = await this.validateEmails(businessData.emailDiscovery.emails);
        stageCost += emailValidation.cost || 0;
        this.totalCost += stageCost;
      } catch (error) {
        console.warn(`Email validation failed for ${businessData.name}:`, error.message);
      }
    }
  ...
  async stage4_EnhancedQualityScoringAndExport(businessData) {
    console.log(`📊 Stage 4: Enhanced quality scoring for ${businessData.name}`);
    ...
    const governmentConfidenceBoost = this.calculateGovernmentConfidenceBoost(businessData);
    
    // Enhanced sector-specific scoring
    const sectorSpecificBoost = this.calculateSectorSpecificBoost(businessData);
    
    // Calculate final confidence score
    const baseScore = Object.values(componentScores).reduce((sum, score) => sum + score, 0) / Object.keys(componentScores).length;
    const finalConfidenceScore = Math.min(100, Math.round(baseScore + governmentConfidenceBoost + sectorSpecificBoost));

    // Enhanced qualification status
    const isQualified = this.determineEnhancedQualificationStatus(businessData, finalConfidenceScore);
  ...
  async validateThroughGovernmentAPIs(business, options = {}) {
    ...
    if (options.enableCaliforniaSOS && this.californiaSOSEnhanced) {
      validationPromises.push(
        this.californiaSOSEnhanced.searchByKeyword(business.name)
    ...
    if (options.enableNonprofit && this.nonprofitClient) {
      validationPromises.push(
        this.nonprofitClient.searchNonprofits(business.name, {
          includeFinancials: options.includeFinancials
        })
    ...
    if (options.enableCompaniesHouseUK && this.companiesHouseClient) {
      validationPromises.push(
        this.companiesHouseClient.searchCompanies(business.name, {
          includeDetails: options.includeFinancials,
          includeOfficers: options.includeOfficers
        })
          .then(result => ({ source: 'companiesHouseUK', result }))
          .catch(error => ({ source: 'companiesHouseUK', error: error.message }))
      );
    }
      ...
      results.forEach(promise => {
        if (promise.status === 'fulfilled' && promise.value.result) {
          ...
          if (result.found && result.totalResults > 0) {
            ...
            if (!validationResults.bestMatch || 
                result.confidenceBoost > validationResults.bestMatch.confidenceBoost) {
    ...
    console.log(`🏛️ Government validation complete: ${validationResults.totalMatches} total matches across APIs`);
  ...
  calculateGovernmentConfidenceBoost(businessData) {
    ...
    Object.keys(this.governmentAPIWeights).forEach(apiKey => {
      ...
      if (apiResult?.found && apiResult.confidenceBoost > 0) {
        const weightedBoost = apiResult.confidenceBoost * this.governmentAPIWeights[apiKey];
        boost += weightedBoost;
      }
    });

    // Additional boost for multiple government sources confirming the business
    const confirmedSources = Object.values(gov.validationSummary || {})
  ...
  calculateEnhancedRiskAssessment(businessData) {
    ...
    if (businessData.governmentValidation?.hasAnyMatch) {
      ...
      if (businessData.governmentValidation?.secEDGAR?.found) {
        riskScore -= 15;
        riskFactors.push('SEC public company filing reduces risk substantially');
      }
    }

    // Multiple government sources reduces risk further
    const govSources = Object.values(businessData.governmentValidation?.validationSummary || {})
      .filter(summary => summary.found).length;
    
    if (govSources > 1) {
      riskScore -= govSources * 5;
      riskFactors.push(`Multiple government sources (${govSources}) confirm business legitimacy`);
    }
  ...
  determineSectorClassification(businessData) {
    ...
    if (gov.companiesHouseUK?.found && gov.companiesHouseUK.companies?.length > 0) {
  ...
  determineEnhancedQualificationStatus(businessData, finalConfidenceScore) {
    ...
    const governmentQualified = finalConfidenceScore >= 65 && governmentBoost >= 15;
    
    // High-value targets (public companies, established nonprofits)
    const highValueTarget = businessData.sectorClassification?.primary === 'Public Company' ||
                           (businessData.sectorClassification?.primary === 'Nonprofit' && 
                            businessData.businessIntelligence?.revenue > 1000000);

    return baseQualified || governmentQualified || (highValueTarget && finalConfidenceScore >= 60);
  }
  ...
  async getBusinessIntelligenceFromGovernmentAPIs(businessData, options) {
    ...
    if (gov.secEDGAR?.companies?.length > 0) {
      ...
      if (company.companyFacts?.financialData) {
        ...
        intelligence.revenue = financials.Revenues?.value || financials.RevenueFromContractWithCustomerExcludingAssessedTax?.value;
  ...
  calculateEnhancedQualityMetrics(results) {
    ...
    return {
      ...base,
      governmentValidated: results.filter(r => r.governmentValidation?.hasAnyMatch).length,
      publicCompanies: results.filter(r => r.sectorClassification?.primary === 'Public Company').length,
      nonprofits: results.filter(r => r.sectorClassification?.primary === 'Nonprofit').length,
      ukCompanies: results.filter(r => r.sectorClassification?.primary === 'UK Company').length,
      averageGovernmentBoost: Math.round(
        results.reduce((sum, r) => sum + (r.governmentConfidenceBoost || 0), 0) / results.length
      ),
      sectorClassified: results.filter(r => r.sectorClassification?.primary !== 'Unknown').length
    };
  }
  ...
  calculateGovernmentAPIMetrics(results) {
    ...
    Object.keys(metrics).forEach(apiKey => {
      if (apiKey !== 'totalGovernmentQueries') {
        ...
        api.hitRate = api.queries > 0 ? Math.round((api.matches / api.queries) * 100) : 0;
  ...
  calculatePreValidationScore(business) {
    ...
    if (business.phone && this.isValidPhoneFormat(business.phone) && !this.isFakePhone(business.phone)) {
      score += 20;
    }

    // Website presence (15 points)
    if (business.website && business.website !== 'http://example.com' && business.website.includes('.')) {
      score += 15;
    }
  ...
  shouldDiscoverEmail(businessData) {
    ...
    const hasGovernmentValidation = businessData.governmentValidation?.hasAnyMatch;
  ...
  async validateBusinessRegistration(business) {
    ...
    try {
      ...
      if (business.state === 'CA' || business.address?.includes('California')) {
        results.california = await this.californiaSOSClient.searchBusiness(business.name);
      }
      
      // New York SOS check  
      if (business.state === 'NY' || business.address?.includes('New York')) {
        results.newYork = await this.newYorkSOSClient.searchBusiness(business.name);
      }
      
      results.registeredInAnyState = Object.values(results).some(r => r?.found);
  ...
  isValidPhoneFormat(phone) {
    return /^\(\d{3}\)\s\d{3}-\d{4}$/.test(phone) || /^\d{3}-\d{3}-\d{4}$/.test(phone);
  }
