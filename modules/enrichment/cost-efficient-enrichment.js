...
class CostEfficientEnrichment {
    constructor(apiKeys = {}) {
        ...
        this.neverBounce = apiKeys.neverBounce ? new NeverBounceClient(apiKeys.neverBounce) : null;
        
        // Cost tracking
        this.costs = {
            hunter: 0.10, // ~$0.10 per email search
    ...
    async enrichBusinessOwnerData(business) {
        ...
        try {
            ...
            console.log('📋 Phase 1: Searching FREE government and open data sources...');
            ...
            if (freeResults.ownerName) {
                ...
                console.log(`✅ Found owner via free sources: ${freeResults.ownerName}`);
                
                // Try to find email using free methods
                const freeEmail = await this.findEmailFreeMethods(freeResults.ownerName, business.website);
                if (freeEmail) {
            ...
            if (this.shouldProceedToLowCost(enrichmentResult.ownerData)) {
                console.log('💳 Phase 2: Proceeding to LOW-COST APIs for email discovery...');
                const lowCostResults = await this.searchLowCostAPIs(business, enrichmentResult.ownerData);
                ...
                if (lowCostResults.emails && lowCostResults.emails.length > 0) {
                    ...
                    enrichmentResult.ownerData.ownerEmail = lowCostResults.emails[0].value;
                    enrichmentResult.ownerData.emailConfidence = lowCostResults.emails[0].confidence;
                    enrichmentResult.ownerData.emailSources = lowCostResults.emails[0].sources;
            ...
            if (enrichmentResult.ownerData?.ownerEmail && this.shouldVerifyEmail(enrichmentResult.ownerData)) {
                console.log('🔍 Phase 3: Verifying email deliverability...');
                const verification = await this.verifyEmailDeliverability(enrichmentResult.ownerData.ownerEmail);
                ...
                if (verification) {
                    ...
                    if (!verification.isValid || verification.confidence < this.qualityThresholds.emailVerificationThreshold) {
            ...
            enrichmentResult.confidenceScore = this.calculateOverallConfidence(enrichmentResult.ownerData);
            enrichmentResult.qualityGrade = this.assignQualityGrade(enrichmentResult.ownerData, enrichmentResult.confidenceScore);
            
            // Update session statistics
            this.updateSessionStats(enrichmentResult);
            
            console.log(`📊 Enrichment complete - Grade: ${enrichmentResult.qualityGrade}, Confidence: ${enrichmentResult.confidenceScore}%, Cost: $${enrichmentResult.actualCost.toFixed(4)}`);
    ...
    async searchLowCostAPIs(business, ownerData) {
        ...
        try {
            ...
            const firstName = ownerData?.ownerName ? this.extractFirstName(ownerData.ownerName) : null;
            const lastName = ownerData?.ownerName ? this.extractLastName(ownerData.ownerName) : null;

            // Search for emails at the business domain
            const hunterResults = await this.hunter.findDomainEmails(domain, firstName, lastName);
    ...
    async verifyEmailDeliverability(email) {
        try {
            if (!this.neverBounce) {
                console.log('⚠️ NeverBounce not configured - skipping email verification');
                return null;
            }
    ...
    assignQualityGrade(ownerData, confidenceScore) {
        ...
        if (confidenceScore >= 90 && ownerData.ownerEmail && ownerData.emailVerification?.isValid) {
            return 'A';
    ...
    getSessionStats() {
        return {
            ...this.sessionStats,
            averageCostPerLead: this.sessionStats.totalCost / Math.max(1, this.sessionStats.successRate.ownerFound),
            totalApiCalls: this.sessionStats.apiCalls.free + this.sessionStats.apiCalls.lowCost + this.sessionStats.apiCalls.expensive
        };
    }
