...
class CaliforniaSOSUnitTests {
  ...
  async testClientInitialization() {
    ...
    try {
      ...
      this.recordTest('Constructor - No API Key', true, 'Client created successfully');
    } catch (error) {
    ...
    try {
      ...
      this.recordTest('Constructor - With API Key', client2.apiKey === 'test-api-key', 'API key set correctly');
    } catch (error) {
    ...
    try {
      ...
      const hasValidDefaults = config.rateLimitPerHour === 100 && 
                              config.qualityScore === 75 && 
                              config.costPerRequest === 0.00;
      this.recordTest('Default Configuration', hasValidDefaults, JSON.stringify(config));
    } catch (error) {
  ...
  async testSearchMethodValidation() {
    ...
    try {
      ...
      this.recordTest('Search - Valid Input', hasRequiredFields, 'Response structure correct');
    } catch (error) {
    ...
    try {
      ...
      this.recordTest('Search - Empty String', false, 'Should reject empty string');
    } catch (error) {
      this.recordTest('Search - Empty String', true, 'Correctly rejects empty input');
    }

    // Test 3: Non-string input
    try {
      await this.client.searchByKeyword(123);
      this.recordTest('Search - Non-String Input', false, 'Should reject non-string input');
    } catch (error) {
      this.recordTest('Search - Non-String Input', true, 'Correctly rejects non-string input');
    }
    ...
    try {
      ...
      this.recordTest('Search - Null Input', true, 'Correctly rejects null input');
    }
  }
  ...
  async testRateLimiting() {
    ...
    try {
      ...
      this.recordTest('Rate Limit - Check Method', typeof canProceed === 'boolean', 
    ...
    try {
      ...
      const canProceed = this.client.checkRateLimit();
      this.recordTest('Rate Limit - Enforcement', !canProceed, 
    ...
    try {
      ...
      this.client.requestCount = this.client.rateLimitPerHour;
      const canProceed = this.client.checkRateLimit();
      this.recordTest('Rate Limit - Window Reset', canProceed, 
  ...
  async testResponseNormalization() {
    ...
    try {
      const normalized = this.client.normalizeSearchResponse([], 'Test Company');
      const isValid = normalized.found === false && 
                     normalized.totalResults === 0 &&
                     normalized.entities.length === 0;
      this.recordTest('Response Normalization - Empty', isValid, 'Empty response handled correctly');
    } catch (error) {
    ...
    try {
      ...
      const normalized = this.client.normalizeSearchResponse(mockData, 'Test Company');
      const isValid = normalized.found === true && 
                     normalized.totalResults === 1 &&
                     normalized.entities[0].entityName === 'Test Company LLC';
      this.recordTest('Response Normalization - Valid Data', isValid, 'Valid data normalized correctly');
    } catch (error) {
    ...
    try {
      ...
      this.recordTest('Response Normalization - Confidence Boost', hasConfidenceBoost, 
        `Confidence boost: ${normalized.confidenceBoost}`);
    } catch (error) {
      this.recordTest('Response Normalization - Confidence Boost', false, error.message);
    }
  }
  ...
  async testErrorHandling() {
    ...
    try {
      ...
      this.recordTest('Error Handling - Network Error', hasErrorHandling, 'Network errors handled gracefully');
    } catch (error) {
      this.recordTest('Error Handling - Network Error', true, 'Error properly thrown and caught');
    }
    ...
    try {
      ...
      this.recordTest('Error Handling - Missing API Key', isMockResponse, 'Mock response returned for missing API key');
    } catch (error) {
  ...
  async testCaching() {
    ...
    try {
      ...
      const key3 = this.client.generateCacheKey('Test Company', { exactMatch: true });
      
      const normalizedSame = key1 === key2;
      const optionsDifferent = key1 !== key3;
      
      this.recordTest('Cache - Key Generation', normalizedSame && optionsDifferent, 
    ...
    try {
      ...
      this.recordTest('Cache - Storage/Retrieval', isCached, 'Cache stores and retrieves data correctly');
    } catch (error) {
    ...
    try {
      ...
      this.recordTest('Cache - Expiration', true, 'Cache expiration handled correctly');
    } catch (error) {
    ...
    try {
      ...
      this.recordTest('Cache - Clear Function', isEmpty, 'Cache cleared successfully');
    } catch (error) {
