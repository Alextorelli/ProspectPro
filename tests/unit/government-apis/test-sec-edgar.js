...
class SECEdgarUnitTests {
  ...
  async testClientInitialization() {
    ...
    try {
      ...
      const hasValidDefaults =
        client.qualityScore === 65 &&
        client.costPerRequest === 0.0 &&
  ...
  async testSearchOperations() {
    ...
    try {
      ...
      try {
        ...
        this.recordTest(
          "Search - Empty String Validation",
          false,
  ...
  async testCompanyFactsRetrieval() {
    ...
    try {
      ...
      try {
        ...
        this.recordTest(
          "Company Facts - CIK Validation",
          false,
          "Should reject invalid CIK"
        );
      } catch (validationError) {
        this.recordTest(
          "Company Facts - CIK Validation",
          true,
          "Correctly validates CIK format"
        );
      }
    } catch (error) {
  ...
  async testRateLimitCompliance() {
    ...
    try {
      ...
      const hasContactInfo =
        userAgent.includes("@") || userAgent.includes("ProspectPro");
  ...
  async testResponseNormalization() {
    ...
    try {
      ...
      const normalized = client.normalizeSearchResponse(
        mockCompanies,
        "Test Company"
      );
      const isValidData =
    ...
    try {
      ...
      this.recordTest(
        "Response Normalization - Confidence Scoring",
        hasConfidenceBoost,
