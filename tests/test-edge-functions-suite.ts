    ...
    for (const testCase of testCases) {
        ...
        try {
            const response = await fetch(
                "http://localhost:8080/business-discovery",
                {
                    method: "POST",
            ...
            if (response.ok) {
                ...
                console.log(
                    `📊 Results: ${
                        result.qualifiedResults || 0
                    } qualified businesses`,
                );
                if (result.businesses && result.businesses.length > 0) {
                    console.log(
                        `📋 Sample: ${result.businesses[0].name} (${
                            result.businesses[0].confidence
                        }% confidence)`,
                    );
                }
            } else {
...
async function testLeadValidation() {
    ...
    for (const testCase of testCases) {
        ...
        try {
            const response = await fetch(
                "http://localhost:8080/lead-validation",
                {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(testCase.request),
            ...
            if (response.ok) {
                console.log(`✅ Status: ${response.status}`);
                console.log(
                    `📊 Results: ${result.qualifiedLeads}/${result.totalLeads} leads qualified`,
                );
                console.log(
                    `📈 Qualification rate: ${result.qualificationRate}%`,
                );

                if (result.results && result.results.length > 0) {
                    const sample = result.results[0];
                    console.log(
                        `📋 Sample: ${sample.name} - Score: ${sample.overallScore}% (${
                            sample.qualified ? "QUALIFIED" : "NOT QUALIFIED"
                        })`,
                    );
                }
            } else {
