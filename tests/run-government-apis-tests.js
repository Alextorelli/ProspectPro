...
class GovernmentAPIsTestRunner {
  ...
  async runAllTests(options = {}) {
    ...
    this.overallResults.duration =
      this.overallResults.endTime - this.overallResults.startTime;
  ...
  async runUnitTests(verbose = false) {
    ...
    try {
      ...
      console.log(
        "⚠️ ProPublica unit tests: Placeholder (implement when needed)"
      );
      ...
      console.error(
        "❌ ProPublica unit tests placeholder failed:",
        error.message
      );
      this.recordSuiteResults("ProPublica Unit Tests", {
        totalTests: 0,
    ...
    try {
      ...
      console.log(
        "⚠️ Companies House UK unit tests: Placeholder (implement when needed)"
      );
      ...
      console.error(
        "❌ Companies House UK unit tests placeholder failed:",
        error.message
      );
      this.recordSuiteResults("Companies House UK Unit Tests", {
        totalTests: 0,
  ...
  async runNetworkDependentTests() {
    ...
    const successRate =
      networkTests.totalTests > 0
        ? Math.round((networkTests.passedTests / networkTests.totalTests) * 100)
        : 100;
  ...
  recordSuiteResults(suiteName, results) {
    ...
    console.log(
      `${status} ${suiteName}: ${results.passedTests || 0}/${
        results.totalTests || 0
      } tests passed (${results.successRate || 0}%)`
    );
  }
  ...
  generateFinalReport() {
    ...
    console.log(`\n📊 Overall Results:`);
    console.log(
      `   Test Suites: ${this.overallResults.passedSuites}/${this.overallResults.totalSuites} passed`
    );
    console.log(
      `   Total Tests: ${this.overallResults.passedTests}/${this.overallResults.totalTests} passed`
    );
    ...
    this.testSuites.forEach((suite) => {
      ...
      console.log(
        `   ${status} ${suite.name}: ${suite.successRate || 0}% (${
          suite.passedTests || 0
        }/${suite.totalTests || 0})`
      );
    });
    ...
    if (this.overallResults.failedTests > 0) {
      ...
      console.log(
        `   ${this.overallResults.failedTests} tests failed across ${this.overallResults.failedSuites} suites`
      );
    ...
    if (overallSuccess >= 95) {
      console.log(
        `   🟢 EXCELLENT: ${overallSuccess}% - Ready for production deployment`
      );
    } else if (overallSuccess >= 85) {
      console.log(
        `   🟡 GOOD: ${overallSuccess}% - Minor issues to address before deployment`
      );
    } else if (overallSuccess >= 70) {
      console.log(
        `   🟠 NEEDS WORK: ${overallSuccess}% - Significant issues require attention`
      );
    } else {
      console.log(
        `   🔴 CRITICAL: ${overallSuccess}% - Major problems must be resolved`
      );
    }
