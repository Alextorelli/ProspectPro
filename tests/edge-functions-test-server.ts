...
async function handleBusinessDiscovery(request: any) {
    ...
    const mockResults =
        mockBusinessData[searchQuery as keyof typeof mockBusinessData] || [];
...
async function handleLeadValidation(request: any) {
        ...
        leads.map(async (lead: any) => {
            ...
            if (!skipEmailValidation && lead.email) {
                ...
                emailValidation = {
                    ...
                    confidence: isValidFormat
                        ? (isCommonProvider ? 85 : 60)
                        : 0,
                    provider: domain,
    ...
    const qualifiedCount =
        validationResults.filter((result) => result.qualified).length;
...
console.log(
    `🚀 ProspectPro Edge Functions Test Server running on http://localhost:${PORT}`,
);
console.log(`📡 Available endpoints:`);
console.log(`   POST http://localhost:${PORT}/business-discovery`);
console.log(`   POST http://localhost:${PORT}/lead-validation`);
console.log(
    `🧪 Run tests with: deno run --allow-net test-edge-functions-suite.ts`,
);
