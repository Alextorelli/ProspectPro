...
const EnhancedLeadDiscoveryWithGovernmentAPIs = require("../modules/enhanced-lead-discovery-with-government-apis");
const ConfidenceScoringAlgorithm = require("../modules/confidence-scoring-algorithm");
...
class Phase1GovernmentAPIsTestSuite {
  ...
  async testCaliforniaSOSUnit() {
    ...
    try {
      ...
      this.recordTest(
        "California SOS - Client Initialization",
        true,
    ...
    try {
      const result = await this.californiaSOSClient.searchByKeyword(
        "Test Company"
      );
      ...
      this.recordTest(
        "California SOS - Response Structure",
        hasRequiredFields,
        hasRequiredFields
    ...
    try {
      ...
      this.recordTest(
        "California SOS - Invalid Input Handling",
        false,
  ...
  async testSECEdgarUnit() {
    ...
    try {
      ...
      this.recordTest(
        "SEC EDGAR - Client Initialization",
        true,
        "Client initialized successfully"
      );
    } catch (error) {
  ...
  async testNonprofitUnit() {
    ...
    try {
      ...
      this.recordTest(
        "Nonprofit API - Client Initialization",
        true,
        "Client initialized successfully"
      );
    } catch (error) {
    ...
    try {
      ...
      const hasRequiredFields =
        ...
        result.hasOwnProperty("qualityScore");
      this.recordTest(
        "Nonprofit API - Response Structure",
        hasRequiredFields,
        hasRequiredFields
          ? "Response has required fields"
          : "Missing required fields"
    ...
    try {
      const result = await this.nonprofitClient.getOrganizationByEIN(
        "530196605"
      );
      const hasEINData = result.found || result.hasOwnProperty("ein");
      this.recordTest(
        "Nonprofit API - EIN Lookup",
        hasEINData,
    ...
    try {
      const classification =
        this.nonprofitClient.getSectorClassification("A20");
      this.recordTest(
        "Nonprofit API - Sector Classification",
  ...
  async testCompaniesHouseUKUnit() {
    ...
    try {
      ...
      this.recordTest(
        "Companies House UK - Client Initialization",
        true,
        "Client initialized successfully"
      );
    } catch (error) {
    ...
    try {
      ...
      const hasRequiredFields =
        ...
        result.hasOwnProperty("qualityScore");
      this.recordTest(
        "Companies House UK - Response Structure",
        hasRequiredFields,
        hasRequiredFields
          ? "Response has required fields"
          : "Missing required fields"
    ...
    try {
      ...
      this.recordTest(
        "Companies House UK - Status Mapping",
        statusDesc === "Active",
        `Status mapping: ${statusDesc}`
      );
    } catch (error) {
      this.recordTest(
        "Companies House UK - Status Mapping",
    ...
    try {
      ...
      const normalized =
        this.companiesHouseClient.normalizeAddress(mockAddress);
      const isValid = normalized && normalized.line1 === "123 Test Street";
      this.recordTest(
        "Companies House UK - Address Normalization",
        isValid,
        isValid
  ...
  async testAPIConnectivity() {
    ...
    try {
      ...
      this.recordTest(
        "California SOS - API Connectivity",
        isConnected,
    ...
    try {
      ...
      const isConnected = !result.error;
      this.recordTest(
        "Nonprofit API - API Connectivity",
        isConnected,
        isConnected ? "API accessible" : "API connection failed"
      );
    } catch (error) {
    ...
    try {
      ...
      const isConnected = !result.error || result.mockData;
      this.recordTest(
        "Companies House UK - API Connectivity",
        isConnected,
        isConnected ? "API accessible" : "API connection failed"
      );
    } catch (error) {
  ...
  async testCrossPlatformData() {
    ...
    this.recordTest(
      "Cross-Platform - Public Company Consistency",
      consistency,
  ...
  async validateDataQuality() {
    ...
    try {
      const caResult = await this.californiaSOSClient.searchByKeyword(
        "Test Company"
      );
    ...
    try {
      ...
      const scoring =
        this.confidenceScoring.calculateConfidenceScore(mockBusiness);
      const hasValidScoring =
        scoring.finalConfidenceScore >= 0 &&
  ...
  async testResponseTimes() {
    ...
    try {
      ...
      this.recordTest(
        "Performance - CA SOS Response Time",
        isAcceptable,
    ...
    try {
      ...
      this.recordTest(
        "Performance - SEC EDGAR Response Time",
        isAcceptable,
        `${responseTime}ms (max: 30000ms)`
      );
    } catch (error) {
  ...
  async testScoringComponents() {
    ...
    try {
      const scoring =
        this.confidenceScoring.calculateConfidenceScore(testBusiness);

      // Test component scores
      const components = scoring.componentScores;
      const allComponentsValid = Object.values(components).every(
        (score) => score >= 0 && score <= 100
      );

      this.recordTest(
        "Confidence Scoring - Component Validity",
        allComponentsValid,
      ...
      this.recordTest(
        "Confidence Scoring - Final Score Range",
        finalScoreValid,
      ...
      const hasQualityTier =
        ...
        [
          ...
          "Poor",
          "Very Poor",
        ].includes(scoring.qualityTier);

      this.recordTest(
        "Confidence Scoring - Quality Tier",
        hasQualityTier,
        `Quality tier: ${scoring.qualityTier}`
      );
    } catch (error) {
      this.recordTest(
        "Confidence Scoring - Component Test",
        false,
        error.message
      );
    }
  ...
  async testGovernmentAPIBoosts() {
    ...
    try {
      const scoring = this.confidenceScoring.calculateConfidenceScore(
        testBusinessWithGovData
      );

      // Test government boost calculation
      const govBoost = scoring.governmentAPIBoost;
      const hasValidBoost =
        govBoost.totalBoost > 0 && govBoost.sourcesFound > 0;

      this.recordTest(
        "Government API - Boost Calculation",
        hasValidBoost,
      ...
      this.recordTest(
        "Government API - Multi-Source Bonus",
        hasMultiSourceBonus,
      ...
      this.recordTest(
        "Government API - Sector Adjustment",
        hasSectorBonus,
  ...
  async testQualityTiers() {
    ...
    testCases.forEach((testCase) => {
      try {
        const tier = this.confidenceScoring.determineQualityTier(
          testCase.score
        );
        const isCorrect = tier === testCase.expectedTier;

        this.recordTest(
          `Quality Tier - Score ${testCase.score}`,
          isCorrect,
  ...
  generateTestReport() {
    ...
    console.log(
      `- California SOS: ${
        this.californiaSOSClient.getUsageStats().totalRequests
      } requests`
    );
    console.log(
      `- SEC EDGAR: ${
        this.secEdgarClient.getUsageStats().totalRequests
      } requests`
    );
    console.log(
      `- Nonprofit API: ${
        this.nonprofitClient.getUsageStats().totalRequests
      } requests`
    );
    console.log(
      `- Companies House UK: ${
        this.companiesHouseClient.getUsageStats().totalRequests
      } requests`
    );
