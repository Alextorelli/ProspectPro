...
server.tool(
  ...
  z.object({
    script: z.string().optional().default('test').describe('npm script to run (default: "test")'),
    timeoutMs: z.number().optional().default(600000).describe('Maximum execution time in milliseconds'),
  }),
  async ({ script = 'test', timeoutMs }) => {
    try {
      const { stdout, stderr } = await runCmd(`npm run -s ${script}`, { timeout: timeoutMs });
...
server.tool(
  ...
  z.object({
    query: z.string().describe('Search term (e.g., "coffee", "restaurant", "Starbucks")'),
    ll: z.string().describe('Latitude,longitude coordinates (e.g., "37.7749,-122.4194")'),
    radius: z.number().optional().default(5000).describe('Search radius in meters (max 100000)'),
    limit: z.number().optional().default(5).describe('Maximum number of results (max 50)'),
    sort: z.string().optional().describe('Sort order: "RELEVANCE" or "DISTANCE"'),
  ...
  async ({ query, ll, radius = 5000, limit = 5, sort, categories }) => {
    ...
    try {
      ...
      return {
        content: [{
          type: 'text',
          text: `✅ Foursquare Search Results (${formatted.length} found):\n\n${JSON.stringify({
            query,
            location: ll,
...
server.tool(
  ...
  async ({ id }) => {
    const token = process.env.FOURSQUARE_SERVICE_API_KEY || process.env.FOURSQUARE_PLACES_API_KEY || '';
    const version = process.env.FOURSQUARE_PLACES_API_VERSION || '2025-06-17';
    ...
    try {
      const url = `https://places-api.foursquare.com/places/${encodeURIComponent(id)}`;
...
server.tool(
  ...
  z.object({
    timeoutMs: z.number().optional().default(30000).describe('Test timeout in milliseconds')
  }),
  async ({ timeoutMs }) => {
    try {
      const { stdout, stderr } = await runCmd('node test/test-foursquare-integration.js', { timeout: timeoutMs });
...
server.tool(
  'health_check',
  'Check the health status of the running ProspectPro server.',
  z.object({
    endpoint: z.string().optional().default('/health').describe('Health endpoint to check (/health, /ready, /live)'),
    host: z.string().optional().default('localhost:3000').describe('Server host and port')
  }),
