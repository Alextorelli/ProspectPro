...
async function runTestCampaign() {
    ...
    try {
        ...
        csvExporter.initializeCampaign(campaignId, {
            ...
            description: "Test campaign for 3 verified leads with comprehensive validation"
        });
        ...
        console.log(`✅ Discovery completed in ${(processingTime / 1000).toFixed(1)}s`);
        console.log("");

        // Filter for highest quality leads
        const highQualityLeads = results
            .filter(business => business.isQualified && business.confidenceScore >= 85)
            .sort((a, b) => b.confidenceScore - a.confidenceScore)
            .slice(0, 3); // Top 3

        const totalCost = results.reduce((sum, business) => sum + (business.apiCost || 0), 0);
        ...
        console.log(`   Success rate: ${((highQualityLeads.length / results.length) * 100).toFixed(1)}%`);
        ...
        highQualityLeads.forEach((lead, index) => {
            ...
            console.log(`   Quality Score: ${lead.confidenceScore}% (${lead.qualityGrade})`);
            ...
            console.log(`     ✓ Business Name: ${lead.validation?.businessName?.isValid ? 'Valid' : 'Invalid'}`);
            console.log(`     ✓ Website Status: ${lead.validation?.website?.isValid ? 'Active' : 'Inactive'} (${lead.validation?.website?.httpStatus || 'N/A'})`);
            console.log(`     ✓ Email Quality: ${lead.validation?.email?.isValid ? 'Verified' : 'Unverified'} (${lead.validation?.email?.confidence || 0}%)`);
        });
        ...
        csvExporter.addQueryResults(
            ...
            highQualityLeads,
            {
                ...
                averageConfidence: highQualityLeads.reduce((sum, lead) => sum + lead.confidenceScore, 0) / highQualityLeads.length
            }
        );
        ...
        try {
            const summaryExists = await require('fs').promises.access(summaryPath).then(() => true).catch(() => false);
            const analysisExists = await require('fs').promises.access(analysisPath).then(() => true).catch(() => false);
        ...
        console.log("=".repeat(60));
        console.log(`✅ High-Quality Leads Found: ${highQualityLeads.length}/3 requested`);
        console.log(`📊 Average Quality Score: ${(highQualityLeads.reduce((sum, lead) => sum + lead.confidenceScore, 0) / highQualityLeads.length).toFixed(1)}%`);
        console.log(`💰 Total Campaign Cost: $${totalCost.toFixed(3)}`);
        console.log(`⏱️  Processing Time: ${(processingTime / 1000).toFixed(1)} seconds`);
        console.log(`📈 Cost per Lead: $${(totalCost / highQualityLeads.length).toFixed(3)}`);
        
        const websiteValid = highQualityLeads.filter(lead => lead.validation?.website?.isValid).length;
        const emailValid = highQualityLeads.filter(lead => lead.validation?.email?.isValid).length;
        const hasOwnerInfo = highQualityLeads.filter(lead => lead.ownerName && lead.ownerName !== lead.businessName).length;
        
        console.log(`🌐 Websites Verified: ${websiteValid}/${highQualityLeads.length} (${((websiteValid/highQualityLeads.length)*100).toFixed(0)}%)`);
        console.log(`📧 Emails Verified: ${emailValid}/${highQualityLeads.length} (${((emailValid/highQualityLeads.length)*100).toFixed(0)}%)`);
        console.log(`👤 Owner Information: ${hasOwnerInfo}/${highQualityLeads.length} (${((hasOwnerInfo/highQualityLeads.length)*100).toFixed(0)}%)`);
