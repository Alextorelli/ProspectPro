...
BEGIN
  IF EXISTS (
    ...
    BEGIN
      -- Enable RLS (may fail in managed environments)
    ...
    EXCEPTION WHEN insufficient_privilege THEN
      -- This is expected in managed Supabase environments
    ...
    WHEN OTHERS THEN
      RAISE NOTICE 'ℹ️  Could not enable RLS on spatial_ref_sys: %', SQLERRM;
    END;
  ELSE
    RAISE NOTICE 'ℹ️  spatial_ref_sys table not found (normal in some environments)';
  END IF;
END $$;
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'ℹ️  Skipped moving pg_trgm: %', SQLERRM;
  END;
  ...
  EXCEPTION WHEN object_not_in_prerequisite_state THEN
    RAISE NOTICE 'ℹ️  PostGIS extension is non-relocatable (normal in managed environments)';
  WHEN insufficient_privilege THEN
    RAISE NOTICE 'ℹ️  Cannot move postgis extension (insufficient privileges)';
  WHEN OTHERS THEN
    RAISE NOTICE 'ℹ️  Skipped moving postgis: %', SQLERRM;
  END;
END $$;
  ...
  FOREACH table_name IN ARRAY tables_to_check LOOP
    IF EXISTS (
      ...
      IF NOT EXISTS (
        ...
        EXECUTE format('CREATE POLICY "Service role can access all %I" ON %I FOR ALL USING (auth.role() = ''service_role'')', table_name, table_name);
        RAISE NOTICE '✅ Enabled RLS on table: %', table_name;
      ELSE
        RAISE NOTICE 'ℹ️  RLS already enabled on table: %', table_name;
      END IF;
    ELSE
      RAISE NOTICE 'ℹ️  Table not found: %', table_name;
    END IF;
  END LOOP;
END $$;
  ...
  FOR rec IN (
    ...
    BEGIN
      ...
      IF NOT EXISTS (
        ...
        RAISE NOTICE '✅ Set search_path on function: %', rec.proname;
      END IF;
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'ℹ️  Could not set search_path on function %: %', rec.proname, SQLERRM;
    END;
  END LOOP;
END $$;
...
BEGIN
  ...
  JOIN pg_namespace n ON n.oid = e.extnamespace
  WHERE n.nspname = 'public'
    AND e.extname NOT IN ('plpgsql', 'uuid-ossp');  -- These are OK in public
  ...
  RAISE NOTICE 'ℹ️  Extensions in public schema: % (some may be non-movable)', extension_count;
