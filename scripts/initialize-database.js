...
async function initializeDatabase() {
  ...
  if (missingVars.length > 0) {
    ...
    log(
      "yellow",
      "💡 Please configure these variables in your .env file or Railway dashboard"
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (connectionError) {
      ...
      log(
        "yellow",
        "💡 Make sure your Supabase project is active and keys are correct"
      );
      process.exit(1);
    }
    ...
    try {
      ...
      if (error && !error.message.includes("null value")) {
        log(
          "yellow",
          `⚠️  Function 'campaign_analytics' may not exist: ${error.message}`
        );
      } else {
      ...
      log(
        "yellow",
        `⚠️  Database functions not fully initialized: ${funcError.message}`
      );
    }
