...
require_repo_root() {
  ...
  if ! repo_root=$(git rev-parse --show-toplevel 2>/dev/null); then
    echo "❌ Unable to determine git root. Run this test from inside the ProspectPro repository." >&2
    exit 1
  fi
  ...
  if [ "$current_dir" != "$repo_root" ]; then
    echo "❌ Run this test from the repository root ($repo_root). Current directory: $current_dir" >&2
    exit 1
  fi

  if [ "$repo_root" != "$expected_root" ]; then
    echo "❌ Repository root mismatch. Expected $expected_root but detected $repo_root." >&2
    echo "   Set EXPECTED_REPO_ROOT to override when working from another checkout." >&2
    exit 1
  fi
}
